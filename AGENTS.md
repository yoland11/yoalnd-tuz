...
12. Production backups and reviewed migrations use dedicated credentials only. Never use the read-only audit credential for a write or migration. A verified backup is required for destructive or materially irreversible work; an unavailable full backup must not block a reviewed, isolated, additive migration.
