-- AJN graduation enterprise extension.
...
CREATE TABLE IF NOT EXISTS graduation_university_profiles (
  ...
  department text,
  logo_url text,
  official_colors jsonb NOT NULL DEFAULT '[]'::jsonb,
  approved_template_ids jsonb NOT NULL DEFAULT '[]'::jsonb,
  recommended_fonts jsonb NOT NULL DEFAULT '[]'::jsonb,
...
CREATE TABLE IF NOT EXISTS graduation_favorites (
  ...
  configuration jsonb NOT NULL DEFAULT '{}'::jsonb,
  created_at timestamp NOT NULL DEFAULT now(),
  CONSTRAINT graduation_favorites_owner_check CHECK (customer_id IS NOT NULL OR session_key IS NOT NULL)
);
...
CREATE TABLE IF NOT EXISTS graduation_measurements (
  ...
  height numeric(7,2), weight numeric(7,2), shoulder numeric(7,2),
  sleeve_length numeric(7,2), chest numeric(7,2), waist numeric(7,2), robe_length numeric(7,2),
  suggested_size varchar(30), confirmed_size varchar(30), notes text,
  version integer NOT NULL DEFAULT 1,
  measured_by integer REFERENCES staff(id) ON DELETE SET NULL,
  created_at timestamp NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS graduation_components (
  ...
  size varchar(30),
  configuration jsonb NOT NULL DEFAULT '{}'::jsonb,
  production_status varchar(40) NOT NULL DEFAULT 'pending',
  packaging_status varchar(40) NOT NULL DEFAULT 'pending',
  is_required boolean NOT NULL DEFAULT true,
  packed_at timestamp,
...
CREATE TABLE IF NOT EXISTS graduation_kits (
  ...
  graduation_order_id integer NOT NULL REFERENCES graduation_orders(id) ON DELETE CASCADE,
  group_id integer REFERENCES graduation_groups(id) ON DELETE SET NULL,
  kit_code varchar(120) NOT NULL,
  qr_value varchar(160) NOT NULL,
  barcode_value varchar(160) NOT NULL,
  status varchar(40) NOT NULL DEFAULT 'awaiting_packaging',
  required_checklist jsonb NOT NULL DEFAULT '{}'::jsonb,
  package_image_url text,
  packaged_by integer REFERENCES staff(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS graduation_kit_items (
  id serial PRIMARY KEY,
  kit_id integer NOT NULL REFERENCES graduation_kits(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS graduation_production_sheets (
  id serial PRIMARY KEY,
  graduation_order_id integer NOT NULL REFERENCES graduation_orders(id) ON DELETE CASCADE,
  sheet_type varchar(40) NOT NULL,
  version integer NOT NULL DEFAULT 1,
...
-- Preserve old orders while materializing their normalized measurement history.
INSERT INTO graduation_measurements (
  graduation_order_id, height, weight, shoulder, sleeve_length, chest, waist,
  suggested_size, confirmed_size, version, created_at
)
SELECT
  o.id,
  NULLIF(o.measurements->>'height','')::numeric,
  NULLIF(o.measurements->>'weight','')::numeric,
  NULLIF(o.measurements->>'shoulder','')::numeric,
  NULLIF(o.measurements->>'sleeveLength','')::numeric,
  NULLIF(o.measurements->>'chest','')::numeric,
  NULLIF(o.measurements->>'waist','')::numeric,
  NULLIF(o.measurements->>'suggestedSize',''),
  COALESCE(NULLIF(o.student_profile->>'size',''), NULLIF(o.measurements->>'confirmedSize','')),
  1,
  o.created_at
FROM graduation_orders o
WHERE o.archived_at IS NULL
ON CONFLICT (graduation_order_id, version) DO NOTHING;

-- Enterprise permissions are additive and inherit the existing graduation gate.
UPDATE staff
SET permissions = (
  SELECT jsonb_agg(DISTINCT permission)
  FROM jsonb_array_elements_text(
    COALESCE(permissions, '[]'::jsonb) ||
    '["graduation.delete","graduation.group.manage","graduation.student.manage","graduation.template.view","graduation.package.manage","graduation.price.edit","graduation.discount.apply","graduation.measurement.update","graduation.preview.manage","graduation.approval.manage","graduation.packaging.scan","graduation.packaging.override","graduation.inventory.view","graduation.reports.view","graduation.settings.manage"]'::jsonb
  ) AS permission
)
WHERE role = 'admin' OR permissions ? 'graduation' OR permissions ? 'graduation_manager';
