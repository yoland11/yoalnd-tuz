...
`schema:preflight`, then `schema:apply` with the exact reviewed migration path and the authoritative Production owner `DATABASE_URL`. The runner accepts safe additive DDL only; destructive changes require a separate safety review.
