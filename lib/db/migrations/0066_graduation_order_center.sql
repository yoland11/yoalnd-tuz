-- Unified individual/group graduation order center.
-- This migration is additive: existing graduation orders remain the canonical
-- student records and receive permanent codes without being moved or deleted.

CREATE TABLE IF NOT EXISTS graduation_templates (
  id serial PRIMARY KEY,
  code varchar(80) NOT NULL,
  name text NOT NULL,
  template_type varchar(40) NOT NULL DEFAULT 'package',
  university text,
  college text,
  department text,
...
SET order_type = CASE WHEN o.group_id IS NULL THEN 'individual' ELSE 'group' END,
    student_code = COALESCE(
      o.student_code,
      CASE
        WHEN o.group_id IS NULL THEN
          'AJN-GR-' || EXTRACT(YEAR FROM o.created_at)::int || '-' || LPAD(o.id::text, 6, '0')
        ELSE
          'AJN-GR-G' || LPAD(o.group_id::text, 3, '0') || '-' || LPAD(o.id::text, 6, '0')
      END
    ),
...
CREATE TABLE IF NOT EXISTS graduation_student_payments (
  id serial PRIMARY KEY,
  payment_batch_id varchar(96) NOT NULL,
...
SELECT o.receipt_no, 'student', o.id, o.group_id,
       jsonb_build_object('orderNo', o.order_no, 'studentCode', o.student_code, 'studentName', o.customer_name,
         'phone', o.phone, 'total', o.total_amount, 'paid', o.paid_amount, 'remaining', o.remaining_amount),
       COALESCE(NULLIF(o.created_by_name, ''), 'النظام'), o.created_at
FROM graduation_orders o
WHERE o.receipt_no IS NOT NULL
...
SET permissions = COALESCE(permissions, '[]'::jsonb) ||
  '["graduation.view","graduation.create","graduation.edit","graduation.group.create","graduation.group.edit","graduation.student.add","graduation.student.delete","graduation.template.manage","graduation.payment.receive","graduation.receipt.print","graduation.production.update","graduation.delivery.confirm","graduation.report.view"]'::jsonb
