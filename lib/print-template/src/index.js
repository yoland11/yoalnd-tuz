...
export function salesInvoiceThermalCss(size, horizontalOffsetMm = 0, verticalOffsetMm = 0) {
  ...
  return `
    ...
    * { box-sizing:border-box; color:#000 !important; text-shadow:none !important; box-shadow:none !important; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
    html, body { width:${size}; margin:0; padding:0; background:#fff !important; }
    ...
    .receipt { width:100%; margin:0 auto; padding:${pad}; ${calibration} }
    ...
    @media print { html,body { width:${size}; margin:0; padding:0; } * { color:#000 !important; } }
  `;
}
...
export function buildSalesInvoiceThermalHtml(input) {
  ...
  return `<!doctype html><html dir="rtl"><head><meta charset="utf-8"><title>${esc(toLatinDigits(input.invoiceNo))}</title><style>${salesInvoiceThermalCss(input.paperSize, input.horizontalOffsetMm, input.verticalOffsetMm)}</style></head><body><div class="receipt">${header}<hr class="rule"><div class="meta-rows">${meta}</div><hr class="rule dashed"><table class="items"><thead>${itemHead}</thead><tbody>${itemRows}</tbody></table>${totals}${notes ? `<hr class="rule dashed"><div class="kv"><span>ملاحظات</span><span class="v">${esc(notes)}</span></div>` : ""}${qr}${footer}</div></body></html>`;
}
