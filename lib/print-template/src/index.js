...
function numericValue(value) {
  ...
  const normalized = toLatinDigits(value)
    .replace(/[\s,،]/g, "")
    .replace(new RegExp(MONEY_LABEL, "g"), "");
...
function paymentStatusLabel(value, paid, remaining) {
  ...
  if (
    status === "paid" ||
    (numericValue(remaining) <= 0 && numericValue(paid) > 0)
  )
...
export function salesInvoiceThermalCss(
  ...
  const customHeight = Math.min(
    500,
    Math.max(40, Number(customHeightMm) || 297),
  );
  const pageSize = isThermal
    ...
    : size === "a4"
      ...
      : size === "a5"
        ? `A5 ${orientation}`
        : `${orientation === "landscape" ? customHeight : customWidth}mm ${orientation === "landscape" ? customWidth : customHeight}mm`;
  const pageMargin = isThermal ? "0" : "10mm";
  const pageWidth = isThermal ? size : "100%";
  // XP-80C-class printers commonly reserve more than the theoretical 4mm
  // side allowance. Keep 8mm on each physical side for a reliable 80mm print.
  const receiptWidth = isThermal ? (is58 ? "52mm" : "64mm") : "100%";
  const pad = isThermal ? "1.5mm" : "0";
  const base = isThermal ? (is58 ? "10px" : "10px") : "12px";
  const qr = isThermal ? (is58 ? "28mm" : "28mm") : "172px";
  const logoSize = isThermal ? (is58 ? "16mm" : "16mm") : "46px";
  ...
  return `
    ...
    * { box-sizing:border-box; color:#000 !important; text-shadow:none !important; box-shadow:none !important; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
    ...
    .r-company { max-width:100%; font-size:14px; font-weight:900; line-height:1.3; overflow-wrap:anywhere; }
...
export function buildSalesInvoiceThermalHtml(input) {
  ...
  const itemRows = items
    .map((item, index) => {
      ...
      const quantity = formatLatinNumber(item.quantity, {
        maximumFractionDigits: 4,
      });
      if (is58)
        return `<tr><td class="name" colspan="2">${esc(name)}</td></tr><tr class="ln2"><td class="num">${quantity} × ${esc(formatLatinMoney(item.unitPrice))}</td><td class="num" style="text-align:left">${esc(formatLatinMoney(item.total))}</td></tr>`;
  ...
  const meta = [
    ...
    input.showCustomerPhone !== false
      ? kv("الهاتف", toLatinDigits(input.customerPhone), "num")
      : "",
    kv("نوع الدفع", paymentMethodLabel(input.paymentMethod)),
    kv(
      "حالة الدفع",
      paymentStatusLabel(input.paymentStatus, input.paid, input.remaining),
    ),
    input.showEmployeeName !== false ? kv("الموظف", input.employeeName) : "",
  ].join("");
  const header = `<div class="r-head">${input.showLogo !== false && input.logoUrl ? `<img class="r-logo" src="${esc(input.logoUrl)}" alt="" onerror="this.remove()">` : ""}<div class="r-company">${esc(input.companyName?.trim() || "مجموعة علي جان نهاد")}</div><div class="r-sub">لتنظيم المناسبات</div><div class="r-sub">${esc(input.documentTitle?.trim() || "فاتورة مبيعات")}</div></div>`;
  ...
  const qr =
    input.showQr !== false && input.qrImageUrl
      ? `<div class="qr"><img src="${esc(input.qrImageUrl)}" alt="QR"><div class="cap num">${esc(toLatinDigits(input.qrCaption || input.invoiceNo))}</div></div>`
      : "";
