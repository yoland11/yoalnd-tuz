...
import { MessageCircle, Printer, Trash2, Plus, Search, X, History, Check } from "lucide-react";
        ...
        : filteredProducts.length === 0 ? <EmptyState message="لا توجد طلبات" /> : (
          <div className="space-y-3">
            {filteredProducts.map(order => {
              ...
              const trackUrl = `${window.location.origin}${import.meta.env.BASE_URL}track?code=${order.trackingCode}`;
                ...
                <div key={order.id} className="bg-card rounded-xl border border-border/30 p-4">
                  <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{order.customerName} — {phone}</p>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <a href={`${import.meta.env.BASE_URL}admin/invoice/${order.id}`} target="_blank" rel="noreferrer"
        ...
        : filteredServices.length === 0 ? <EmptyState message="لا توجد حجوزات" /> : (
          <div className="space-y-3">
            {filteredServices.map(o => {
              ...
              const trackUrl = `${window.location.origin}${import.meta.env.BASE_URL}track?code=${o.trackingCode}`;
                ...
                <div key={o.id} className={`bg-card rounded-xl border p-4 ${isReschedulePending ? "border-amber-500/50 ring-1 ring-amber-500/30" : "border-border/30"}`}>
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{o.customerName} — {o.phone}</p>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <a href={`${import.meta.env.BASE_URL}admin/invoice/${o.id}?type=booking`} target="_blank" rel="noreferrer"
                      className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-primary/10 text-primary border border-primary/30 hover:bg-primary/20">
      ...
      <div className="bg-card border border-border/40 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={e => {
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input label="رقم الهاتف *" value={form.customerPhone} onChange={v => setForm(f => ({ ...f, customerPhone: v }))} required />
