 ...
 * through the site's CSS tokens (`--primary`, `--background`, `--border`).
 *
 * Items come from the shared `navigationItems` source (dock surface), plus a
 * "More" entry that opens a bottom sheet (vaul Drawer) listing the remaining
 * desktop links — so the dock/sheet and the desktop navbar never drift.
 */
...
const ITEMS: Item[] = [
  ...dockNavItems.map((i) => ({
    key: i.key, href: i.href, label: i.label, Icon: i.Icon, match: i.match,
    translate: i.translate, badge: i.key === "cart",
  })),
  { key: "more", href: "", label: "المزيد", Icon: MoreHorizontal, match: isSheetRoute, translate: false, isMore: true },
];

// Geometry (px). TOPPAD is the transparent band above the pill the bump/circle
// overflow into; it also matches the wrapper's padding-top. Unchanged from the
// original 5-item dock — only the item count grew.
const DOCK_H = 64;
const CIRCLE = 54;
...
export function MobileNav() {
  ...
  const label = (it: { translate: boolean; label: string }) => (it.translate ? t(it.label) : it.label);
  ...
  const paint = useCallback(() => {
    const { W, centers } = geoRef.current;
  ...
  const tick = useCallback((t2: number) => {
    if (!last.current) last.current = t2;
  ...
  const openMore = () => { const idx = activeFromLocation(); const c = geoRef.current.centers[idx]; if (c != null) { spring.current.target = c; kick(); } setSheetOpen(true); };
  ...
  useEffect(() => {
    ...
    const onUp = () => {
      ...
      if (item.isMore) { openMore(); return; }        // opens the sheet, snaps back to the active route
      navigate(item.href);
  ...
  const go = (i: number) => {
    ...
    if (item.isMore) { openMore(); return; }
    navigate(item.href);
  };
  const onKey = (e: React.KeyboardEvent) => {
    if (e.key === "ArrowLeft") { go(isRTL ? active + 1 : active - 1); e.preventDefault(); }
    else if (e.key === "ArrowRight") { go(isRTL ? active - 1 : active + 1); e.preventDefault(); }
  };
  ...
  return (
    <>
          ...
          >
              ...
              {ITEMS.map((it, i) => {
                ...
                return (
                  <button
                    ...
                    aria-label={label(it)}
                    aria-haspopup={it.isMore ? "dialog" : undefined}
                    tabIndex={-1}
                    onClick={() => go(i)}
            ...
            >
              {ITEMS.map((it, i) => {
                const Icon = it.Icon;
                return (
                  <span key={it.key} className="absolute grid place-items-center transition-opacity duration-150" style={{ opacity: i === active ? 1 : 0 }}>
      ...
      {/* "More" bottom sheet — remaining desktop links (vaul Drawer). */}
      <Drawer open={sheetOpen} onOpenChange={setSheetOpen} shouldScaleBackground={false}>
        <DrawerContent dir={dir} className="lg:hidden">
          <DrawerTitle className="px-4 pb-1 pt-2 text-center">{t("المزيد")}</DrawerTitle>
          <nav aria-label="روابط إضافية" className="grid grid-cols-3 gap-2 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))]">
