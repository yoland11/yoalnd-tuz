          ...
          <Card className="border-border bg-card">
            <CardContent className="p-6 md:p-7">
              ...
              <div className="mt-3 flex items-baseline gap-3">
                <span className="text-2xl font-bold text-primary">{formatKoshaPrice(kosha.price)}</span>
                {kosha.oldPrice ? <span className="text-sm text-muted-foreground line-through">{formatKoshaPrice(kosha.oldPrice)}</span> : null}
                {Number(kosha.discountPercentage ?? 0) > 0 ? <span className="rounded-full bg-primary/10 px-2 py-1 text-xs font-bold text-primary">خصم {kosha.discountPercentage}%</span> : null}
              </div>
