...
type DesignerView = DesignerSection|"gallery";
...
const galleryKey=["/api/gallery","bouquet-designer-gallery"] as const;
...
async function fetchDesignerGallery():Promise<GalleryItem[]>{const response=await fetch("/api/gallery",{credentials:"include"});if(!response.ok)throw new Error("تعذر تحميل معرض الصور");return response.json();}
...
export default function FlowerDesigner(){
 ...
 const [quantities,setQuantities]=useState<Record<string,number>>({}); const [order,setOrder]=useState<string[]>([]); const [chosen,setChosen]=useState<Record<number,number|undefined>>({}); const [draftQty,setDraftQty]=useState<Record<number,number>>({}); const [note,setNote]=useState(""); const [templateName,setTemplateName]=useState(""); const [details,setDetails]=useState<CatalogProduct|null>(null); const [summaryOpen,setSummaryOpen]=useState(false); const [adding,setAdding]=useState(false); const [limit,setLimit]=useState(24); const [galleryCategory,setGalleryCategory]=useState("all"); const [galleryPreview,setGalleryPreview]=useState<GalleryItem|null>(null);
 ...
 const gallery=useQuery({queryKey:galleryKey,queryFn:fetchDesignerGallery,enabled:active==="gallery",staleTime:60_000,refetchOnWindowFocus:true});
 ...
 return <main dir="rtl" className="flower-design-studio"><div className="flower-design-studio__shell">
  ...
  <nav className="flower-design-studio__categories" aria-label="فئات مصمم الباقات">
   ...
   <button type="button" data-section="gallery" className={cn("flower-design-studio__category",active==="gallery"&&"is-active")} onClick={()=>setActive("gallery")}><span className="flower-design-studio__category-icon" aria-hidden="true">🖼️</span><span>معرض الصور</span></button>
  </nav>
  <div className="flower-design-studio__catalog-title"><div><h2>{active==="gallery"?"معرض الصور":LABEL[active]}</h2><p>{active==="gallery"?"استكشف صور الباقات واختر القسم الذي يهمك.":`${filtered.length} منتج متاح ضمن مصمم الباقات`}</p></div></div>
  {active==="gallery"?<DesignGallery categories={galleryCategories} selectedCategory={galleryCategory} items={galleryItems} isLoading={gallery.isLoading} isError={gallery.isError} onCategoryChange={setGalleryCategory} onPreview={setGalleryPreview} onRetry={()=>void gallery.refetch()}/>:catalog.isLoading?<CatalogSkeleton/>:catalog.isError?<Empty text="تعذر تحميل كتالوج المصمم" action={()=>void catalog.refetch()}/>:visible.length?<div className="flower-design-studio__product-grid">{visible.map(product=><ProductCard key={product.id} product={product} chosen={chosen[product.id]} quantity={draftQty[product.id]??1} onChoose={variant=>setChosen(current=>({...current,[product.id]:variant.id}))} onQuantity={value=>setDraftQty(current=>({...current,[product.id]:value}))} onAdd={()=>product.designerSection==="ready_bouquets"||product.isReadyMadeBouquet||product.isBouquetTemplate?applyTemplate(product):addProduct(product)} onDetails={()=>setDetails(product)}/>)}</div>:<Empty text="لا توجد نتائج مطابقة لعوامل البحث والتصفية." action={()=>{setSearch("");setAvailability("all");setColor("all");setPrice("all");}} actionLabel="مسح التصفية"/>}{active!=="gallery"&&filtered.length>limit&&<Button variant="outline" className="mt-5 w-full" onClick={()=>setLimit(value=>value+24)}>عرض المزيد</Button>}</section>
...
function DesignGallery({categories,selectedCategory,items,isLoading,isError,onCategoryChange,onPreview,onRetry}:{categories:string[];selectedCategory:string;items:GalleryItem[];isLoading:boolean;isError:boolean;onCategoryChange:(category:string)=>void;onPreview:(item:GalleryItem)=>void;onRetry:()=>void}){return <section className="flower-design-studio__gallery" aria-label="معرض الصور"><div className="flower-design-studio__gallery-toolbar"><div className="flower-design-studio__gallery-categories" role="group" aria-label="أقسام معرض الصور">{categories.map(category=>{const label=category==="all"?"كل الصور":category;return <button key={category} type="button" aria-pressed={selectedCategory===category} className={cn("flower-design-studio__gallery-category",selectedCategory===category&&"is-active")} onClick={()=>onCategoryChange(category)}>{label}</button>;})}</div><Button type="button" variant="outline" className="flower-design-studio__gallery-manage" asChild><Link href="/admin/gallery"><ImagePlus/>إدارة وإضافة الصور</Link></Button></div>{isLoading?<GallerySkeleton/>:isError?<Empty text="تعذر تحميل معرض الصور" action={onRetry}/>:items.length?<div className="flower-design-studio__gallery-grid">{items.map(item=>{const title=item.titleAr||item.title||"صورة من معرض الباقات";return <button key={item.id} type="button" className="flower-design-studio__gallery-card" onClick={()=>onPreview(item)} aria-label={`عرض ${title}`}><span className="flower-design-studio__gallery-media">{item.mediaType==="video"?<><video src={item.mediaUrl} muted playsInline preload="metadata" aria-hidden="true"/><span className="flower-design-studio__gallery-play" aria-hidden="true"><Play/></span></>:<img src={item.mediaUrl} alt="" loading="lazy" decoding="async"/>}</span><span className="flower-design-studio__gallery-caption"><b>{title}</b>{item.category&&<small>{item.category}</small>}</span></button>;})}</div>:<Empty text="لا توجد صور ضمن هذا القسم حتى الآن."/>}</section>}
