...
import { Copy, Flower2, ImageOff, Minus, Package, Plus, RefreshCw, Search, ShoppingBag, SlidersHorizontal, Trash2, X } from "lucide-react";
...
export default function FlowerDesigner(){
 ...
 return <main dir="rtl" className="flower-design-studio"><div className="flower-design-studio__shell">
  ...
  <nav className="flower-design-studio__categories" aria-label="فئات مصمم الباقات">{enabled.map(section=><button key={section} type="button" data-section={section} className={cn("flower-design-studio__category",active===section&&"is-active")} onClick={()=>setActive(section)}><span className="flower-design-studio__category-icon" aria-hidden="true">{CATEGORY_ICON[section]}</span><span>{LABEL[section]}</span></button>)}</nav>
  <div className="flower-design-studio__catalog-title"><div><h2>{LABEL[active]}</h2><p>{filtered.length} منتج متاح ضمن مصمم الباقات</p></div></div>
  {catalog.isLoading?<CatalogSkeleton/>:catalog.isError?<Empty text="تعذر تحميل كتالوج المصمم" action={()=>void catalog.refetch()}/>:visible.length?<div className="flower-design-studio__product-grid">{visible.map(product=><ProductCard key={product.id} product={product} chosen={chosen[product.id]} quantity={draftQty[product.id]??1} onChoose={variant=>setChosen(current=>({...current,[product.id]:variant.id}))} onQuantity={value=>setDraftQty(current=>({...current,[product.id]:value}))} onAdd={()=>product.designerSection==="ready_bouquets"||product.isReadyMadeBouquet||product.isBouquetTemplate?applyTemplate(product):addProduct(product)} onDetails={()=>setDetails(product)}/>)}</div>:<Empty text="لا توجد نتائج مطابقة لعوامل البحث والتصفية." action={()=>{setSearch("");setAvailability("all");setColor("all");setPrice("all");}} actionLabel="مسح التصفية"/>}{filtered.length>limit&&<Button variant="outline" className="mt-5 w-full" onClick={()=>setLimit(value=>value+24)}>عرض المزيد</Button>}</section>
