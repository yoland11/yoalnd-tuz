                  ...
                  {command.data.upcoming.slice(0, 10).map((row) => (
                    <Link key={`${row.type}-${row.id}`} href={row.href} className="rounded-lg border border-border/30 bg-background/40 p-3 transition-colors hover:border-primary/40">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className="shrink-0 rounded-full bg-primary/10 px-2 py-1 text-[11px] text-primary">{STATUS_LABELS[row.status] ?? row.status}</span>
                      </div>
...
const CAL_TYPE_COLOR: Record<string, string> = { out: "bg-accent", in: "bg-status-success", maintenance: "bg-status-warning" };
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-3 text-[11px] text-muted-foreground">
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-accent" /> خروج</span>
