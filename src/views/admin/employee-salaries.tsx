...
function EmployeeSalariesPageInner() {
  ...
  function submitEditor(): void {
    ...
    if (editor.mode === "create") {
      ...
      createMutation.mutate({ manual: true, standalone: form.joinCycle !== "1", employeeId, period: form.period, periodStartDate: form.periodStartDate, periodEndDate: form.periodEndDate, paymentDate: form.paymentDate || null, baseSalary: n(form.baseSalary), allowances: n(form.allowances), bonusAmount: n(form.bonusAmount), overtimeAmount: n(form.overtimeAmount), manualAddition: n(form.manualAddition), manualDeduction: n(form.manualDeduction), advanceDeduction: n(form.advanceDeduction), paymentMethod: form.paymentMethod || "cash", notes: form.notes || null });
      return;
    }
  ...
  return <main className="mx-auto w-full max-w-[1700px] space-y-5 p-3 sm:p-5" dir="rtl">
    ...
    <Card className="overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[820px] text-sm"><thead className="bg-muted/60 text-xs"><tr>{["الموظف", "الفترة", "الأساسي", "الصافي", "المدفوع", "المتبقي", "الحالة", "الإجراءات"].map((h) => <th key={h} className="whitespace-nowrap px-3 py-2.5 text-start font-semibold">{h}</th>)}</tr></thead><tbody className="divide-y">
      {runsQuery.isLoading && <tr><td colSpan={8} className="p-12 text-center text-muted-foreground"><Loader2 className="mx-auto mb-3 h-6 w-6 animate-spin" />جارٍ تحميل سجلات الرواتب…</td></tr>}
      ...
      {!runsQuery.isLoading && !filtered.length && <tr><td colSpan={8} className="p-14 text-center"><WalletCards className="mx-auto mb-3 h-8 w-8 text-muted-foreground" /><b>لا توجد رواتب مطابقة</b><p className="mt-1 text-sm text-muted-foreground">غيّر البحث أو الفلاتر لعرض سجلات أخرى.</p></td></tr>}
    </tbody><tfoot className="border-t-2 bg-muted/40 font-semibold"><tr><td className="whitespace-nowrap px-3 py-3.5">{filtered.length.toLocaleString("ar-IQ-u-nu-latn")} سجل راتب</td><td className="px-3 py-3.5" />{[totals.base, totals.net, totals.paid, totals.remaining].map((v, i) => <td key={i} className="whitespace-nowrap px-3 py-3.5 text-xs">{money.format(v)}</td>)}<td colSpan={2} /></tr></tfoot></table></div></Card>
...
function MoneyCell({ value, strong, tone = "" }: { value: number; strong?: boolean; tone?: string }) { return <td className={`whitespace-nowrap px-3 py-2.5 tabular-nums ${strong ? "font-bold" : ""} ${tone}`}>{money.format(n(value))}</td>; }
...
function SalaryDetails({ row, management, loading, statementError, savingStatementPdf, onClose, onPrint, onPrintStatement, onSaveStatementPdf, onReversePayment, onAttach, onCancelMovement }: { row: SalaryRow | null; management?: SalaryManagement; loading: boolean; statementError: boolean; savingStatementPdf: boolean; onClose: () => void; onPrint: (row: SalaryRow) => void; onPrintStatement: (row: SalaryRow) => void; onSaveStatementPdf: (row: SalaryRow) => void; onReversePayment: (payment: SalaryManagement["payments"][number]) => void; onAttach: () => void; onCancelMovement: (adjustment: SalaryManagement["adjustments"][number]) => void }) {
  if (!row) return null;
  // A loaded management payload may miss an array (legacy/partial). Never call an
  // array method on a possibly-undefined field — that throw was blanking the
  // whole details dialog behind the error boundary.
  const mgmtPayments = Array.isArray(management?.payments) ? management!.payments : [];
  const mgmtAdjustments = Array.isArray(management?.adjustments) ? management!.adjustments : [];
  const mgmtEvents = Array.isArray(management?.events) ? management!.events : [];
  const mgmtAttachments = Array.isArray(management?.attachments) ? management!.attachments : [];
  const allowances = n(row.attendanceAllowance) + n(row.transportationAllowance) + n(row.foodAllowance) + n(row.phoneAllowance) + n(row.housingAllowance) + n(row.otherFixedAllowances);
  ...
  return <Dialog open onOpenChange={(open) => !open && onClose()}><DialogContent className="max-h-[92vh] max-w-4xl overflow-y-auto" dir="rtl"><DialogHeader><DialogTitle className="flex flex-wrap items-center gap-2">تفاصيل الراتب <Badge variant="outline">{row.salaryNumber}</Badge></DialogTitle><DialogDescription>{row.employeeName} · {row.employeeCode} · {periodLabel(row.period)}</DialogDescription></DialogHeader>
    ...
    <DetailsBoundary key={row.id}><Tabs defaultValue="calculation"><TabsList className="h-auto w-full flex-wrap justify-start"><TabsTrigger value="calculation">تفاصيل احتساب الراتب</TabsTrigger><TabsTrigger value="components">مكونات الراتب</TabsTrigger><TabsTrigger value="payments">المدفوعات</TabsTrigger><TabsTrigger value="adjustments">التعديلات</TabsTrigger><TabsTrigger value="accounting">المحاسبة والصندوق</TabsTrigger><TabsTrigger value="history">التدقيق والخط الزمني</TabsTrigger><TabsTrigger value="attachments">المرفقات</TabsTrigger></TabsList>
      ...
      <TabsContent value="history" className="space-y-2"><InfoRows rows={[["أنشئ بواسطة", row.createdBy || "النظام القديم"], ["اعتمد بواسطة", row.approvedBy || "—"], ["سجلات الدورة", `${row.auditLog?.length || 0} تدقيق · ${row.timeline?.length || 0} خط زمني`]]} />{loading ? <LoadingBlock /> : mgmtEvents.length ? mgmtEvents.map((event) => <div key={event.id} className="rounded-xl border p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><b>{String(event.action ?? "").replaceAll("_", " ")}</b><span className="text-xs text-muted-foreground">{event.created_at ? new Date(event.created_at).toLocaleString("ar-IQ-u-nu-latn") : "—"}</span></div><p className="mt-1 text-muted-foreground">{event.actor_name}{event.reason ? ` · ${event.reason}` : ""}</p></div>) : <EmptyBlock text="لا توجد أحداث تفصيلية إضافية." />}</TabsContent>
      <TabsContent value="attachments" className="space-y-2">{loading ? <LoadingBlock /> : mgmtAttachments.length ? mgmtAttachments.map((attachment) => <a key={attachment.id} href={attachment.data_url} download={attachment.name} className="flex items-center gap-3 rounded-xl border p-3 transition-colors hover:bg-muted/50"><Paperclip className="h-4 w-4" /><div className="min-w-0 flex-1"><b className="block truncate text-sm">{attachment.name}</b><span className="text-xs text-muted-foreground">{attachment.uploaded_by_name} · {attachment.created_at ? new Date(attachment.created_at).toLocaleString("ar-IQ-u-nu-latn") : "—"}</span></div><Download className="h-4 w-4" /></a>) : <EmptyBlock text="لا توجد مرفقات مسجلة لهذا الراتب." />}<Button variant="outline" onClick={onAttach}><Paperclip className="ms-2 h-4 w-4" />إضافة مرفق</Button></TabsContent>
    </Tabs></DetailsBoundary>
...
function PayrollCalculationDetails({ row, management }: { row: SalaryRow; management?: SalaryManagement }) {
  // Guard against a management payload that loaded but lacks the arrays (legacy
  // rows, partial responses): the optional chain only guarded `management`, so a
  // truthy object without `adjustments` threw and blanked the whole details tab.
  const formulas = (row.calculationDetails?.formulas && typeof row.calculationDetails.formulas === "object") ? row.calculationDetails.formulas : {};
  const attendance = (row.calculationDetails?.attendance && typeof row.calculationDetails.attendance === "object") ? row.calculationDetails.attendance : {};
  const mgmtAdjustments = Array.isArray(management?.adjustments) ? management!.adjustments : [];
  const allowances = n(row.attendanceAllowance) + n(row.transportationAllowance) + n(row.foodAllowance) + n(row.phoneAllowance) + n(row.housingAllowance) + n(row.otherFixedAllowances);
  ...
  const values: Array<{ label: string; value: number; formula?: string; tone?: "add" | "deduct" | "net" }> = [
    ...
    { label: "الإضافات اليدوية والعمولة", value: additions, formula: mgmtAdjustments.filter((item) => item.direction === "addition").map((item) => `${item.reason}: ${money.format(n(item.amount))}`).join(" · ") || undefined, tone: "add" },
    ...
    { label: "الخصومات اليدوية", value: row.manualDeduction, formula: mgmtAdjustments.filter((item) => item.direction === "deduction").map((item) => `${item.reason}: ${money.format(n(item.amount))}`).join(" · ") || (row.calculationDetails?.manualEdit?.reason ? `تعديل المدير: ${row.calculationDetails.manualEdit.reason}` : undefined), tone: "deduct" },
  ...
  return <div className="space-y-4"><div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">{metrics.map(([label, value]) => <div key={label} className="rounded-lg bg-muted/50 p-2"><span className="text-xs text-muted-foreground">{label}</span><b className="mt-1 block tabular-nums">{n(value).toLocaleString("en-US", { maximumFractionDigits: 2 })}</b></div>)}</div><div className="divide-y rounded-xl border">{values.map((item) => <div key={item.label} className="p-3"><div className="flex items-center justify-between gap-4"><span className="text-sm text-muted-foreground">{item.label}</span><b className={`tabular-nums ${item.tone === "add" ? "text-emerald-700 dark:text-emerald-300" : item.tone === "deduct" ? "text-rose-700 dark:text-rose-300" : item.tone === "net" ? "text-primary" : ""}`}>{money.format(item.value)}</b></div>{typeof item.formula === "string" && item.formula.trim() ? <p className="mt-1 text-xs leading-5 text-muted-foreground">{item.formula}</p> : null}</div>)}</div>{row.calculationDetails?.originalCalculated && <div className="rounded-lg bg-muted/40 p-3 text-xs text-muted-foreground">القيم الأصلية المحسوبة محفوظة في سجل الراتب، وأي تعديل يدوي يظهر مع المدير والوقت والسبب في سجل التدقيق.</div>}</div>;
}
