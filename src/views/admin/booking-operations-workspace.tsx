  ...
  return <div className="ajn-booking-operations" dir="rtl">
    ...
    <header className="ajn-op-sticky">
      <div className="ajn-op-header-main">
        <div className="ajn-op-identity"><span className="ajn-op-mark"><Sparkles /></span><div><div className="ajn-op-title"><h1>{booking.number}</h1><OperationStatus value={overview.data?.bookingStage ?? booking.status} /></div><p><UserRound /> {booking.customerName} <span>·</span> {booking.phone}</p></div></div>
        <div className="ajn-op-facts">
          ...
          <span><CircleDollarSign /><b>{money(booking.remaining)}</b><small>المبلغ المتبقي</small></span>
          <span><Warehouse /><b>{STAGE_LABELS[overview.data?.warehouseStage ?? "reserved"]}</b><small>حالة المستودع</small></span>
        </div>
        <div className="ajn-op-header-actions"><Button variant="outline" asChild><Link href={invoiceUrl}><Printer /> الفاتورة</Link></Button><Button className="ajn-op-primary" asChild><Link href={paymentUrl}><Banknote /> استلام دفعة</Link></Button></div>
      </div>
    </header>
...
function ProductsTab({ base, queryKey }: { base: string; queryKey: unknown[] }) {
  ...
  return <div className="ajn-op-tab-panel"><div className="ajn-op-section-head"><div><ShoppingBag /><span><small>المتجر والمخزون</small><h2>منتجات الحجز</h2></span></div><div className="ajn-op-actions"><OperationStatus value={products.data?.stage ?? "draft"} /><Button variant="outline" onClick={() => action.mutate("release")} disabled={action.isPending || !lines.length}><RotateCcw /> تحرير الحجز</Button><Button className="ajn-op-primary" onClick={() => save.mutate()} disabled={save.isPending || !lines.length}>{save.isPending ? <Loader2 className="animate-spin" /> : <Check />} حفظ وحجز المخزون</Button></div></div><div className="ajn-op-picker"><Search /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="ابحث باسم المنتج أو الباركود" /><div className="ajn-op-picker-results">{catalog.data?.data?.slice(0, 8).map((item) => <button key={item.id} type="button" onClick={() => add(item)} disabled={item.available <= 0}><span><b>{item.name}</b><small>{item.barcode || item.category || "منتج متجر"}</small></span><span><strong>{item.available}</strong><small>متاح</small></span><Plus /></button>)}</div></div><QueryState loading={products.isLoading} error={products.error} empty={!lines.length}><div className="ajn-op-table-wrap"><table className="ajn-op-table"><thead><tr><th>المنتج</th><th>اللون / المتغير</th><th>الكمية</th><th>المتاح</th><th>سعر الوحدة</th><th>الخصم</th><th>الإجمالي</th><th>الحالة</th><th aria-label="إزالة" /></tr></thead><tbody>{lines.map((line, index) => { const item = catalog.data?.data.find((entry) => entry.id === line.productId); return <tr key={`${line.productId}:${line.variantId ?? 0}:${line.id}`}><td><b>{line.productName}</b><small>{line.barcode}</small></td><td><select value={line.variantId ?? ""} onChange={(event) => { const variantId = event.target.value ? Number(event.target.value) : null; const variant = item?.variants.find((entry) => entry.id === variantId); update(index, { variantId, variantLabel: variant ? [variant.color, variant.size].filter(Boolean).join(" / ") : null, available: variant?.available ?? item?.available ?? line.available }); }}><option value="">بدون متغير</option>{item?.variants.map((variant) => <option key={variant.id} value={variant.id}>{[variant.color, variant.size].filter(Boolean).join(" / ")} · متاح {variant.available}</option>)}</select></td><td><Input type="number" min={1} value={line.quantity} onChange={(event) => update(index, { quantity: Math.max(1, Number(event.target.value) || 1) })} /></td><td className={line.quantity > line.available ? "is-danger" : ""}>{line.available}</td><td><Input type="number" min={0} value={line.unitPrice} onChange={(event) => update(index, { unitPrice: Math.max(0, Number(event.target.value) || 0) })} /></td><td><Input type="number" min={0} value={line.discount} onChange={(event) => update(index, { discount: Math.max(0, Number(event.target.value) || 0) })} /></td><td>{money(Math.max(0, line.unitPrice * line.quantity - line.discount))}</td><td><OperationStatus value={line.status} /></td><td><Button variant="ghost" size="icon" aria-label={`إزالة ${line.productName}`} onClick={() => setDraft(lines.filter((_, lineIndex) => lineIndex !== index))}>×</Button></td></tr>; })}</tbody><tfoot><tr><td colSpan={6}>إجمالي منتجات الحجز</td><td>{money(lines.reduce((sum, line) => sum + Math.max(0, line.unitPrice * line.quantity - line.discount), 0))}</td><td colSpan={2} /></tr></tfoot></table></div></QueryState></div>;
}
...
function AssetsTab({ base, queryKey }: { base: string; queryKey: unknown[] }) {
  ...
  return <div className="ajn-op-tab-panel"><div className="ajn-op-section-head"><div><PackageCheck /><span><small>دورة حياة الأصل</small><h2>أصول الحجز</h2></span></div><div className="ajn-op-actions"><Badge variant="outline">{assets.data?.assets.length ?? 0} أصل</Badge><Button variant="outline" asChild><Link href="/admin/assets">فتح إدارة الأصول</Link></Button></div></div><div className="ajn-op-picker"><Search /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="ابحث عن أصل بالاسم أو الباركود" /><div className="ajn-op-picker-results">{catalog.data?.data.filter((item) => item.isAsset).slice(0, 8).map((item) => <button key={item.id} type="button" onClick={() => mutation.mutate({ mode: "link", productId: item.id, quantity: 1 })} disabled={mutation.isPending || ["maintenance", "lost", "retired", "locked"].includes(String(item.profileStatus))}><span><b>{item.name}</b><small>{item.barcode || "أصل تشغيلي"}</small></span><span><strong>{item.available}</strong><small>متاح</small></span><Plus /></button>)}</div></div><QueryState loading={assets.isLoading} error={assets.error} empty={!assets.data?.assets.length}><div className="ajn-op-asset-list">{assets.data?.assets.map((row) => <article key={row.productId}><div className="ajn-op-asset-main"><span className="ajn-op-asset-icon"><PackageCheck /></span><div><div><h3>{row.name}</h3><OperationStatus value={row.stage} /></div><p>{row.assetCode}{row.serialNumber ? ` · ${row.serialNumber}` : ""}{row.warehouse ? ` · ${row.warehouse}` : ""}</p></div></div><div className="ajn-op-asset-stats"><span><small>الإجمالي</small><b>{row.quantity}</b></span><span><small>المتاح</small><b>{row.available}</b></span><span><small>المحجوز</small><b>{row.reserved}</b></span><span><small>الخارج</small><b>{row.out}</b></span><span><small>المرتجع</small><b>{row.returned}</b></span><span className={row.damaged || row.missing ? "is-danger" : ""}><small>تلف / نقص</small><b>{row.damaged + row.missing}</b></span><span><small>الصحة</small><b>{row.healthScore}%</b></span></div><div className="ajn-op-asset-actions">{row.stage === "linked" && <Button size="sm" onClick={() => act(row, "reserve")}>حجز الأصل</Button>}{row.stage === "reserved" && <Button size="sm" onClick={() => act(row, "pick")}><Barcode /> تجهيز</Button>}{row.stage === "picked" && <Button size="sm" onClick={() => act(row, "checkout")}><QrCode /> إخراج</Button>}{row.stage === "out" && <Button size="sm" onClick={() => { setReturning(row); setReturnForm({ problem: "none", description: "", estimatedCost: "", usageHours: "", managerApproval: false }); }}><RotateCcw /> تسجيل الإرجاع</Button>}{row.stage === "returned" && <Button size="sm" onClick={() => mutation.mutate({ mode: "inspect", productId: row.productId, quantity: row.quantity, usageHours: Number(returnForm.usageHours || 0), problem: row.problem || "none" })}><ClipboardCheck /> إكمال الفحص</Button>}<Button variant="ghost" size="sm" asChild><Link href={`/admin/products?focus=${row.productId}`}>جواز الأصل</Link></Button>{!row.out && <Button variant="ghost" size="sm" onClick={() => act(row, "unlink")}>إزالة</Button>}</div></article>)}</div></QueryState>{returning && <section className="ajn-op-return-panel"><div><RotateCcw /><span><small>فحص الإرجاع</small><h3>{returning.name}</h3></span><Button variant="ghost" onClick={() => setReturning(null)}>إغلاق</Button></div><div className="ajn-op-return-grid"><div><Label htmlFor="asset-problem">هل يوجد تلف أو نقص؟</Label><select id="asset-problem" value={returnForm.problem} onChange={(event) => setReturnForm({ ...returnForm, problem: event.target.value })}><option value="none">لا، الأصل سليم</option><option value="damaged">نعم، يوجد تلف</option><option value="missing">نعم، يوجد نقص / فقدان</option></select></div><div><Label htmlFor="asset-hours">ساعات الاستخدام</Label><Input id="asset-hours" type="number" min={0} value={returnForm.usageHours} onChange={(event) => setReturnForm({ ...returnForm, usageHours: event.target.value })} /></div>{returnForm.problem !== "none" && <><div className="sm:col-span-2"><Label htmlFor="asset-description">وصف الحالة *</Label><Textarea id="asset-description" value={returnForm.description} onChange={(event) => setReturnForm({ ...returnForm, description: event.target.value })} placeholder="اكتب تفاصيل التلف أو الجزء المفقود" /></div><div><Label htmlFor="asset-cost">التكلفة التقديرية</Label><Input id="asset-cost" type="number" min={0} value={returnForm.estimatedCost} onChange={(event) => setReturnForm({ ...returnForm, estimatedCost: event.target.value })} /></div>{returnForm.problem === "missing" && <label className="ajn-op-check"><input type="checkbox" checked={returnForm.managerApproval} onChange={(event) => setReturnForm({ ...returnForm, managerApproval: event.target.checked })} /> اعتماد المدير على تسجيل النقص</label>}</>}</div><Button className="ajn-op-primary" disabled={mutation.isPending || (returnForm.problem !== "none" && !returnForm.description.trim())} onClick={() => mutation.mutate({ mode: "return", productId: returning.productId, quantity: returning.quantity, problem: returnForm.problem, description: returnForm.description, estimatedCost: Number(returnForm.estimatedCost || 0), managerApproval: returnForm.managerApproval })}>تأكيد الإرجاع وإرسال الأصل للفحص</Button></section>}</div>;
}
...
function FinanceTab({ base, queryKey, booking, invoiceUrl, paymentUrl }: { base: string; queryKey: unknown[]; booking: BookingOperationsBooking; invoiceUrl: string; paymentUrl: string }) {
  ...
  const summary = [["المبلغ الأصلي", data?.total ?? booking.total], ["منتجات المتجر", data?.productCharges ?? 0], ["رسوم التلف والنقص", data?.damageCharges ?? 0], ["المبلغ النهائي", data?.finalAmount ?? booking.total], ["المدفوع", data?.paid ?? booking.paid], ["المتبقي", data?.remaining ?? booking.remaining], ["التكلفة التقديرية", data?.estimatedCost ?? 0], ["الربح التقديري", data?.estimatedProfit ?? 0]];
