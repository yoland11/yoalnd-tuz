...
function AssetsTab({ base, queryKey }: { base: string; queryKey: unknown[] }) {
  ...
  return <div className="ajn-op-tab-panel ajn-op-assets-panel">
    ...
    <div className="ajn-op-picker"><Search /><Input id="booking-asset-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="ابحث عن أصل بالاسم أو الباركود" /><div className="ajn-op-picker-results">{catalog.data?.data.filter((item) => item.isAsset).slice(0, 8).map((item) => <button key={item.id} type="button" onClick={() => mutation.mutate({ mode: "link", productId: item.id, quantity: 1 })} disabled={mutation.isPending || ["maintenance", "lost", "retired", "locked"].includes(String(item.profileStatus))}><span><b>{item.name}</b><small>{item.barcode || "أصل تشغيلي"}</small></span><span><strong>{item.available}</strong><small>متاح</small></span><Plus /></button>)}</div></div>
