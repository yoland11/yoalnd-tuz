...
const cards = (data: ExecutiveData) => [
  ...
  ["الموظفون الحاضرون", Number(data.presentToday ?? 0).toLocaleString("ar-IQ-u-nu-latn"), "/admin/attendance", Users],
  ["الموظفون الغائبون", Number(data.absentToday ?? 0).toLocaleString("ar-IQ-u-nu-latn"), "/admin/attendance", Users],
  ["سلف الموظفين", formatCurrency(data.outstandingAdvances), "/admin/employee-advances", Banknote],
  ["الحجوزات النشطة", Number(data.activeBookings ?? 0).toLocaleString("ar-IQ-u-nu-latn"), "/admin/kosha-bookings", CalendarDays],
  ["المخزون المنخفض", Number(data.lowStock ?? 0).toLocaleString("ar-IQ-u-nu-latn"), "/admin/inventory-alerts", AlertTriangle],
  ["أصول تحت الصيانة", Number(data.assetsUnderMaintenance ?? 0).toLocaleString("ar-IQ-u-nu-latn"), "/admin/asset-reports", AlertTriangle],
] as const;
