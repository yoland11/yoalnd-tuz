...
export function openGraduationLabelPrintWindow({
  ...
  const html = `<!doctype html><html dir="rtl"><head><meta charset="utf-8"><title>${itemType} - ${studentCode}</title><style>${graduationLabelCss(paperSize)}</style></head><body>
    <main class="graduation-label"><section><div class="brand">AJN · ${AJN_COMPANY_NAME}</div><div class="name">${studentName}</div><div class="meta">${itemType}${size ? ` · ${size}` : ""}${color ? ` · ${color}` : ""}${group ? `<br>${group}` : ""}</div><div class="code">${studentCode}</div></section><img src="${qrDataUrl}" alt="QR"></main>
    ${printWhenImagesReadyScript()}
  </body></html>`;
