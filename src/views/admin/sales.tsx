    ...
    try {
      ...
      if (printOutput?.remotePrintOptions && savedInvoice?.id) {
        try {
          ...
          toast({ title: "تم حفظ الفاتورة لكن تعذر إرسالها للطباعة", description: apiErrorMessage(printError), variant: "destructive" });
        }
      }
                  ...
                  {held.map(h => (
                    <div key={h.id} className="flex items-center justify-between bg-muted/30 rounded-lg p-3">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{h.items.length} صنف · {new Date(h.createdAt).toLocaleTimeString("ar-u-nu-latn")}</p>
                      </div>
...
function CustomerAccountStatementTab({ invoice }: { invoice: SalesInvoice }) {
  ...
  return <section className="space-y-4" aria-label="كشف حساب العميل">
    ...
    <div className="overflow-x-auto rounded-xl border border-border/30 bg-background/40">
      <table className="w-full min-w-[940px] text-sm"><thead><tr className="border-b border-border/30 bg-muted/30 text-xs text-muted-foreground"><th className="p-3 text-right">رقم العملية</th><th className="p-3 text-right">التاريخ</th><th className="p-3 text-right">نوع الخدمة</th><th className="p-3 text-left">الإجمالي</th><th className="p-3 text-left">المدفوع</th><th className="p-3 text-left">المتبقي</th><th className="p-3 text-right">سجل الدفعات</th></tr></thead><tbody className="divide-y divide-border/20">{filteredTransactions.length ? filteredTransactions.map((row) => <tr key={`${row.serviceType}-${row.reference}-${row.date}`} className="hover:bg-muted/10"><td className="p-3 font-mono text-xs text-primary" dir="ltr">{row.reference}</td><td className="p-3 text-xs">{new Date(row.date).toLocaleDateString("ar-IQ-u-nu-latn")}</td><td className="p-3">{row.serviceType}</td><td className="p-3 text-left tabular-nums" dir="ltr">{formatCurrency(row.total)}</td><td className="p-3 text-left tabular-nums text-status-success" dir="ltr">{formatCurrency(row.paid)}</td><td className="p-3 text-left tabular-nums font-semibold" dir="ltr">{formatCurrency(row.remaining)}</td><td className="p-3 text-xs text-muted-foreground">{row.paymentHistory?.length ? row.paymentHistory.map((payment, index) => <div key={`${payment.reference}-${payment.date}-${index}`} className="mb-1 last:mb-0"><span dir="ltr">{new Date(payment.date).toLocaleDateString("ar-IQ-u-nu-latn")}</span> · <strong className="text-status-success" dir="ltr">{formatCurrency(payment.amount)}</strong></div>) : "—"}</td></tr>) : <tr><td colSpan={7} className="p-10 text-center text-sm text-muted-foreground">{fromDate || toDate ? "لا توجد عمليات ضمن الفترة المحددة." : "لا توجد عمليات لهذا العميل."}</td></tr>}</tbody></table>
    </div>
  </section>;
}
  ...
  async function directPrintInvoice() {
    ...
    try {
      const result = await adminFetch<{ job: RemotePrintJob; duplicate: boolean }>("/admin/print-jobs", {
        method: "POST",
        headers: { "idempotency-key": remotePrintIdempotencyKey(invoice.id, "direct") },
  ...
  async function reprintDirectInvoice() {
    ...
    try {
      const result = await adminFetch<{ job: RemotePrintJob; duplicate: boolean }>(`/admin/print-jobs/${lastPrintJob.id}/reprint`, {
        method: "POST",
        headers: { "idempotency-key": remotePrintIdempotencyKey(invoiceId, "direct") },
          ...
          <div className="overflow-y-auto max-h-[calc(92vh-76px)] p-5 space-y-4">
            ...
            <section className="rounded-xl border border-border/30 bg-card p-4" aria-label="معلومات الموظف">
              ...
              <dl className="grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div><dt className="text-xs text-muted-foreground">تاريخ ووقت الإنشاء</dt><dd className="mt-1 font-medium">{invoice.createdAt ? new Date(invoice.createdAt).toLocaleString("ar-IQ-u-nu-latn", { dateStyle: "short", timeStyle: "short" }) : "—"}</dd></div>
                <div><dt className="text-xs text-muted-foreground">آخر تعديل</dt><dd className="mt-1 font-medium">{invoice.updatedAt ? new Date(invoice.updatedAt).toLocaleString("ar-IQ-u-nu-latn", { dateStyle: "short", timeStyle: "short" }) : "—"}</dd></div>
              </dl>
            </section>
