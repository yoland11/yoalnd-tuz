    ...
    <div ref={reportRef} className="space-y-5" dir="rtl">
      ...
      <div className="overflow-hidden rounded-xl border border-border/30 bg-card">
        <div className="grid sm:grid-cols-3 lg:grid-cols-6">
          ...
          <Stat label="بانتظار الموافقة" value={`${data.pending.count.toLocaleString("ar-IQ")} · ${formatCurrency(data.pending.amount)}`} icon={ShieldCheck} tone="text-amber-300" />
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[1fr_1fr_1.4fr]">
        ...
        <div className="rounded-xl bg-card p-4"><p className="text-xs text-muted-foreground">ذمم الزبائن</p><div className="mt-3 flex items-end justify-between gap-4"><div><p className="text-xs text-muted-foreground">المتبقي</p><p className="mt-1 font-bold text-foreground">{formatCurrency(data.outstanding)}</p></div><div className="text-left"><p className="text-xs text-muted-foreground">المتأخر</p><p className="mt-1 font-bold text-amber-300">{formatCurrency(data.overdue)}</p></div></div></div>
        <div className="rounded-xl bg-card p-4"><div className="flex items-center justify-between"><p className="text-xs text-muted-foreground">التلف والخسائر هذا الشهر</p><TriangleAlert className="h-4 w-4 text-amber-300" /></div><p className="mt-3 text-xl font-bold text-foreground">{formatCurrency(data.damageLosses)}</p><p className="mt-1 text-xs text-muted-foreground">تُسجل مع المنتج والموظف المسؤول ضمن حركة مالية مدققة.</p></div>
      </div>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        ...
        <TabsContent value="overview" className="space-y-4">
          <div className="grid gap-4 xl:grid-cols-[1.35fr_1fr]">
            <div className="rounded-xl border border-border/30 bg-card p-4"><h2 className="mb-4 font-semibold text-foreground">اتجاه الإيرادات والمصاريف</h2>{data.trend.length === 0 ? <EmptyState message="لا توجد حركات منفذة لعرض الاتجاه" /> : <div className="h-72" dir="ltr"><ResponsiveContainer width="100%" height="100%"><BarChart data={data.trend}><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis dataKey="month" tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} /><YAxis tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} /><Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8 }} formatter={(value) => formatCurrency(Number(value))} /><Legend /><Bar dataKey="revenue" name="الإيرادات" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} /><Bar dataKey="expenses" name="المصاريف" fill="hsl(var(--destructive))" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></div>}</div>
      ...
      <Dialog open={selectedId !== null} onOpenChange={(open) => !open && setSelectedId(null)}><DialogContent className="max-h-[88vh] max-w-3xl overflow-y-auto" dir="rtl"><DialogHeader><DialogTitle>تفاصيل الحركة المالية</DialogTitle></DialogHeader>{detail.isLoading || !detail.data ? <Skeleton className="h-72 rounded-xl" /> : <TransactionDetailView data={detail.data} />}</DialogContent></Dialog>
    </div>
  );
}
