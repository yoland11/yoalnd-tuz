...
type FinancialTransaction = {
  ...
  cashDirectionLabel?: string;
  accountingClassification?: "revenue" | "expense" | "liability_inflow" | "liability_repayment";
  accountingClassificationLabel?: string;
  amount: string;
  department: string;
...
const movementDisplay = (row: Pick<FinancialTransaction, "direction" | "transactionType" | "sourceType" | "cashDirectionLabel" | "accountingClassificationLabel">) => {
  if (row.accountingClassificationLabel && row.cashDirectionLabel) return { direction: row.cashDirectionLabel, classification: row.accountingClassificationLabel, isLiability: row.accountingClassificationLabel.includes("التزام") || row.accountingClassificationLabel.includes("تخفيض") };
  if (row.transactionType === "company_loan_received" || row.sourceType === "company_loan") return { direction: "وارد", classification: "استلام قرض / التزام", isLiability: true };
  if (row.transactionType === "company_loan_repayment" || row.sourceType === "company_loan_repayment") return { direction: "صادر", classification: "تسديد قرض / تخفيض التزام", isLiability: true };
  return { direction: row.direction === "revenue" ? "وارد" : "صادر", classification: row.direction === "revenue" ? "إيراد" : "مصروف", isLiability: false };
};
...
function downloadCsv(rows: FinancialTransaction[]) {
  ...
  const values = rows.map((row) => [
    ...
    row.transactionType,
    `${movementDisplay(row).direction} — ${movementDisplay(row).classification}`,
    row.amount,
    ...
    <div ref={reportRef} className="space-y-5" dir="rtl">
      ...
      <div className="overflow-hidden rounded-xl border border-border/30 bg-card">
        <div className="grid sm:grid-cols-3 lg:grid-cols-6">
          ...
          <Stat label="بانتظار الموافقة" value={`${data.pending.count.toLocaleString("ar-IQ-u-nu-latn")} · ${formatCurrency(data.pending.amount)}`} icon={ShieldCheck} tone="text-status-warning" onClick={() => setMcTab("approvals")} />
        </div>
      </div>
      ...
      <Tabs value={mcTab} onValueChange={setMcTab} className="space-y-4">
        <TabsList className="print:hidden"><TabsTrigger value="overview">الملخص</TabsTrigger><TabsTrigger value="ledger">دفتر الحركات</TabsTrigger><TabsTrigger value="approvals">الموافقات ({(pendingApprovals.data?.total ?? data.pending.count).toLocaleString("ar-IQ-u-nu-latn")})</TabsTrigger></TabsList>
        ...
        {APPROVAL_FILTERS.map((item) => (
          ...
          >
            ...
            <span className={`rounded-full px-1.5 text-[11px] ${filter === item.value ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"}`}>
              {(counts[item.value] ?? 0).toLocaleString("ar-IQ-u-nu-latn")}
            </span>
          </button>
        ))}
