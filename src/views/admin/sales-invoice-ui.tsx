  ...
  return (
    <section className={`${surface} overflow-hidden`}>
      ...
      {!items.length ? <div className="grid min-h-56 place-items-center p-8 text-center"><div><span className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-[#FBF5F3] text-[#B85C65]"><ShoppingCart className="h-7 w-7" /></span><h3 className="mt-4 font-semibold text-[#182033]">لم يتم إضافة أي منتجات بعد</h3><p className="mt-1 max-w-sm text-sm text-[#778092]">ابحث عن منتج بالاسم أو امسح الباركود أو أضف منتجاً يدوياً</p></div></div> : <>
        ...
        <div className="space-y-3 p-3 md:hidden">{items.map((line, index) => <article key={`${line.barcode}-${index}`} className="rounded-2xl border border-[#E9E5E2] bg-[#FCFBFA] p-3"><div className="flex gap-3"><LineThumbnail source={imageForLine?.(line)} alt={line.productName} /><div className="min-w-0 flex-1"><input aria-label={`اسم المنتج ${index + 1}`} value={line.productName} onChange={e => onUpdate(index, "productName", e.target.value)} className="w-full bg-transparent font-semibold text-[#182033] outline-none" />{line.barcode ? <p className="mt-1 text-xs text-[#778092]" dir="ltr">{line.barcode}</p> : null}</div><button type="button" aria-label={`حذف ${line.productName}`} disabled={disabled} onClick={() => onRemove(index)} className="h-10 text-[#D75A5A]"><Trash2 className="h-4 w-4" /></button></div><div className="mt-3 grid grid-cols-2 gap-3"><label className="block"><span className="mb-1 block text-xs text-[#778092]">سعر الوحدة</span><input aria-label={`سعر الوحدة ${line.productName}`} type="number" inputMode="decimal" min="0" step="0.001" value={line.unitPrice} disabled={disabled} onChange={e => onUpdate(index, "unitPrice", e.target.value)} className={`${input} w-full text-center font-semibold`} /></label><div><span className="mb-1 block text-xs text-[#778092]">الكمية</span><div className="flex items-center justify-between rounded-xl border border-[#E9E5E2] bg-white p-1"><button type="button" aria-label="تقليل الكمية" disabled={disabled} className="h-8 w-8" onClick={() => quantity(index, line.quantity - 1)}>−</button><input aria-label={`كمية ${line.productName}`} type="number" min="0.001" step="0.001" value={line.quantity} disabled={disabled} onChange={e => onUpdate(index, "quantity", e.target.value)} className="w-14 bg-transparent text-center outline-none" /><button type="button" aria-label="زيادة الكمية" disabled={disabled} className="h-8 w-8 text-[#B85C65]" onClick={() => quantity(index, line.quantity + 1)}>+</button></div></div></div><div className="mt-3 flex items-center justify-between border-t border-[#E9E5E2]/70 pt-3"><span className="text-xs text-[#778092]">الإجمالي</span><strong className="text-[#182033]">{formatMoney(line.total)}</strong></div></article>)}</div>
      </>}
    </section>
  );
}
