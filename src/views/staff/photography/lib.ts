...
export const photographyApi = {
  ...
  events: (search = "") => adminFetch<PhotographyEvent[]>(`${base}/events?search=${encodeURIComponent(search)}`),
  event: (ref: string) => adminFetch<PhotographyEvent>(`${base}/events/${encodeURIComponent(ref)}`),
  ...
  orders: (search = "", status = "") => adminFetch<PhotographyOrder[]>(`${base}/orders?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}`),
  ...
  reports: () => adminFetch<{ orders: number; delivered: number; inProgress: number; received: number; remaining: number }>(`${base}/reports`),
