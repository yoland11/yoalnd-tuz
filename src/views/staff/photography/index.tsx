...
import {
  ...
  Radio, Send, Trash2, Undo2, Upload, User, Users, WalletCards, X,
} from "lucide-react";
...
import { apiErrorMessage, apiErrorStatus, fetchAdminMe, hasPerm, isSessionDecision, loginAdmin, logoutAdmin, type AdminMe } from "@/views/admin/_lib";
...
import { ShootBoardPage, ShootDetailPage, ShootsListPage } from "./shoots";
import { EditingPage, MemoryCardsPage, OpsReportPage } from "./post";
    ...
    try { await loginAdmin(username.trim(), password); onDone(); }
      ...
      if (isSessionDecision(err)) {
        if (typeof window !== "undefined" && window.confirm(err.message)) {
          try { await loginAdmin(username.trim(), password, { forceReplace: true }); onDone(); }
          catch (retryErr: any) { setError(String(retryErr?.message ?? "بيانات الدخول غير صحيحة").replace(/^HTTP\s+\d+:\s*/, "")); }
        }
      } else {
        setError(String(err?.message ?? "بيانات الدخول غير صحيحة").replace(/^HTTP\s+\d+:\s*/, ""));
      }
    }
...
function Dashboard({ me }: { me: AdminMe }) {
  ...
  return <div className="space-y-5"><ShootBoardPage /><div className="space-y-5 p-4"><EventForm me={me} />{data ? <><div className="grid grid-cols-2 gap-2"><Stat label="مناسبات اليوم" value={data.counts.events} icon={CalendarDays} /><Stat label="طلبات اليوم" value={data.counts.orders} icon={ClipboardList} /><Stat label="جاهز للاستلام" value={data.counts.ready} icon={PackageCheck} /><Stat label="تم التسليم" value={data.counts.delivered} icon={Check} /></div>{data.recentOrders.length ? <section><div className="mb-2 flex items-center justify-between"><h2 className="text-sm font-bold">آخر الطلبات</h2><Link href="/staff/photography/orders" className="text-xs text-primary">عرض الكل</Link></div><div className="space-y-2">{data.recentOrders.slice(0, 3).map((order) => <OrderCard key={order.id} order={order} compact />)}</div></section> : null}</> : <div className="py-6 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin text-primary" /></div>}</div></div>;
}
...
export default function PhotographyStaffPortal() {
  ...
  const tabs = [
    ...
    { href: "/staff/photography/shoots", label: "المهام", icon: Radio, active: location.startsWith("/staff/photography/shoots") },
    ...
    { href: "/staff?tab=salary", label: "راتبي", icon: WalletCards, active: false },
  ...
  return <div className="min-h-dvh bg-background text-foreground" dir="rtl"><header className="sticky top-0 z-20 flex min-h-16 items-center justify-between gap-3 border-b border-border/30 bg-background/95 px-4 py-3 backdrop-blur"><div className="flex min-w-0 items-center gap-2"><Camera className="h-5 w-5 flex-shrink-0 text-primary" /><div className="min-w-0"><div className="truncate text-sm font-bold text-foreground">{me.fullName || me.username}</div><div className="truncate text-[11px] text-muted-foreground">بوابة المصورين{isPhotoManager(me) ? " • مدير" : ""}</div></div></div><div className="flex flex-shrink-0 items-center gap-2">{pendingOps > 0 ? <span className="flex items-center gap-1 rounded-full bg-status-warning/15 px-2 py-1 text-[11px] font-bold text-status-warning"><CloudOff className="h-3.5 w-3.5" />{pendingOps}</span> : null}<button type="button" onClick={() => navigate("/staff/photography/account")} aria-label="الحساب والأجهزة" className="grid h-10 w-10 place-items-center text-muted-foreground"><User className="h-5 w-5" /></button><button type="button" onClick={() => logoutAdmin().then(refreshMe)} aria-label="تسجيل الخروج" className="grid h-10 w-10 place-items-center text-muted-foreground"><LogOut className="h-5 w-5" /></button></div></header><main className="mx-auto max-w-3xl pb-24"><Switch><Route path="/staff/photography/editing"><EditingPage me={me} /></Route><Route path="/staff/photography/cards"><MemoryCardsPage me={me} /></Route><Route path="/staff/photography/ops-reports"><OpsReportPage /></Route><Route path="/staff/photography/shoots/:ref">{(params) => <ShootDetailPage shootRef={decodeURIComponent(params.ref as string)} me={me} />}</Route><Route path="/staff/photography/shoots"><ShootsListPage /></Route><Route path="/staff/photography/events/:token/register">{(params) => <RegistrationPage eventRef={params.token} me={me} />}</Route><Route path="/staff/photography/orders/:id">{(params) => <OrderDetailPage id={Number(params.id)} me={me} />}</Route><Route path="/staff/photography/events"><EventsPage me={me} /></Route><Route path="/staff/photography/orders"><OrdersPage me={me} /></Route><Route path="/staff/photography/delivery"><OrdersPage delivery me={me} /></Route><Route path="/staff/photography/reports"><ReportsPage me={me} /></Route><Route path="/staff/photography/notifications"><NotificationsPage /></Route><Route path="/staff/photography/account"><div className="p-4"><div className="mb-4"><div className="text-sm font-bold text-foreground">{me.fullName || me.username}</div><div className="text-[11px] text-muted-foreground">بوابة المصورين</div></div><SessionDevicesPanel portal="photography" onSwitched={refreshMe} /></div></Route><Route path="/staff/photography"><Dashboard me={me} /></Route><Route><Dashboard me={me} /></Route></Switch></main><nav className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-8 border-t border-border/30 bg-background/95 pb-[env(safe-area-inset-bottom)] backdrop-blur">{tabs.map((tab) => <button key={tab.href} type="button" onClick={() => navigate(tab.href)} aria-current={tab.active ? "page" : undefined} className={`relative flex min-h-16 min-w-0 flex-col items-center justify-center gap-1 text-[11px] transition-colors ${tab.active ? "font-bold text-primary" : "text-muted-foreground"}`}>{tab.active ? <span className="absolute inset-x-4 top-0 h-0.5 rounded-full bg-primary" /> : null}<tab.icon className={`h-5 w-5 transition-transform ${tab.active ? "scale-110" : ""}`} /><span className="max-w-full truncate px-1">{tab.label}</span>{tab.badge ? <span className="absolute right-[24%] top-1.5 grid h-4 min-w-4 place-items-center rounded-full bg-destructive px-1 text-[11px] font-bold text-white">{tab.badge}</span> : null}</button>)}</nav></div>;
}
