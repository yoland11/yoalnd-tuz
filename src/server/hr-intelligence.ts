...
export async function ensureHrTables() {
  ...
  await db.execute(sql`
    ...
    create unique index if not exists payroll_runs_active_period_uq on payroll_runs(period) where deleted_at is null;
...
async function buildPayroll(input: unknown, persist: boolean, actor?: HrActor, replaceRunId?: number) {
  ...
  const existing = rows<any>(await db.execute(sql`select * from payroll_runs where period=${data.period} and deleted_at is null limit 1`))[0];
  ...
  const [attendanceResult, eventResult] = await Promise.all([
    ...
    db.execute(sql`select staff_id, kind, coalesce(sum(amount::numeric),0)::float as amount from hr_incentive_events where period=${data.period} and status='applied_to_payroll' group by staff_id,kind`),
  ]);
  ...
  for (const l of lines) {
    ...
    if (line && l.incentive.bonus > 0) await db.execute(sql`update hr_incentive_events set payroll_line_id=${line.id} where staff_id=${l.employee.id} and period=${data.period} and status='applied_to_payroll' and payroll_line_id is null and kind in ('bonus','reward')`);
  }
