...
async function createAutomaticTasksForEntity(input: {
  ...
  for (const [index, template] of templates.entries()) {
    ...
    await createNotification({
      ...
      body: row.title,
      staffId: assignedStaffIds[0] ?? null,
      entityType: "task",
      entityId: row.id,
      href: "/admin/tasks",
  ...
  if (section === "disaster-recovery") {
    ...
    if (method === "POST") {
      const counts = await Promise.all([
        ...
        db.select({ c: sql<number>`count(*)::int` }).from(entityDocumentsTable),
      ]);
      const [draft] = await db.insert(disasterRecoverySnapshotsTable).values({
        snapshotNo: `BKP-TMP-${randomUUID()}`,
        type: "manual",
        status: "created",
        summary: { orders: counts[0][0]?.c ?? 0, products: counts[1][0]?.c ?? 0, customers: counts[2][0]?.c ?? 0, documents: counts[3][0]?.c ?? 0 },
        createdBy: auth.id,
        createdByName: auth.fullName || auth.username,
      }).returning();
      const snapshotNo = `BKP-${String(draft.id).padStart(6, "0")}`;
      const [row] = await db.update(disasterRecoverySnapshotsTable).set({ snapshotNo }).where(eq(disasterRecoverySnapshotsTable.id, draft.id)).returning();
      void logAdminActivity(req, "disaster_recovery_snapshot_created", "disaster_recovery_snapshot", row.id, { snapshotNo });
