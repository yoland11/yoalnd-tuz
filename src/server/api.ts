...
function formatTask(row: any, staffById = new Map<number, any>(), entityProgress?: { total: number; completed: number; percent: number } | null) {
...
function erpActorFromAdmin(user?: AdminUser | null): ErpActor {
  return { id: user?.id ?? null, name: user ? (user.fullName || user.username) : "النظام" };
}
...
function erpEntityHref(entityType: string, entityId?: number | null): string {
  ...
  if (entityType === "photography_event") return entityId ? `/staff/photography/events/${entityId}/register` : "/staff/photography";
  if (entityType === "photography_order") return entityId ? `/staff/photography/orders/${entityId}` : "/staff/photography/orders";
...
async function addEntityTimeline(input: {
  ...
  await ensureAdminExtensionsTables();
  const [row] = await db.insert(entityTimelineTable).values({
...
function eventDateToDueAt(value: string | Date | null | undefined, stepIndex: number, totalSteps: number): Date | null {
  if (!value) return null;
  const base = value instanceof Date ? value : new Date(`${String(value).slice(0, 10)}T09:00:00`);
...
function taskTemplateForEntity(entityType: string, serviceType?: string | null) {
  const normalized = `${entityType} ${serviceType ?? ""}`.toLowerCase();
  if (/photography|photo|تصوير|album|ألبوم/.test(normalized)) {
    return [
      { key: "photography_capture", title: "التصوير", description: "تنفيذ جلسة التصوير وتأكيد حضور الكادر." },
      { key: "photography_editing", title: "المونتاج", description: "متابعة المونتاج وفرز الصور المطلوبة." },
      { key: "photography_print", title: "الطباعة", description: "تجهيز الطباعة أو الألبوم حسب طلب الزبون." },
      { key: "photography_delivery", title: "التسليم", description: "تسليم الصور وتأكيد حالة الدفع." },
    ];
  }
  if (/kosha|كوش/.test(normalized)) {
    return [
      { key: "kosha_prepare", title: "التجهيز", description: "تجهيز القطع والاكسسوارات الخاصة بالحجز." },
      { key: "kosha_load", title: "التحميل", description: "تحميل المواد وتوثيق خروجها من المخزن." },
      { key: "kosha_install", title: "التنصيب", description: "تنصيب الكوشة في موقع المناسبة." },
      { key: "kosha_remove", title: "الفك", description: "فك المواد بعد انتهاء المناسبة." },
      { key: "kosha_return", title: "الإرجاع", description: "إرجاع المواد وفحص النقص أو التلف." },
    ];
  }
  return [
    { key: "booking_prepare", title: "تجهيز الحجز", description: "مراجعة تفاصيل الحجز والتأكد من جاهزية المتطلبات." },
    { key: "booking_execute", title: "تنفيذ الحجز", description: "تنفيذ الخدمة حسب بيانات الزبون." },
    { key: "booking_followup", title: "متابعة التسليم", description: "تأكيد التسليم وحالة الدفع." },
  ];
}
...
async function createAutomaticTasksForEntity(input: {
  ...
  const assignedStaffIds = Array.from(new Set((input.assignedStaffIds ?? []).filter((id) => Number.isInteger(id) && id > 0)));
  const created: any[] = [];
  for (const [index, template] of templates.entries()) {
    ...
    await Promise.all((assignedStaffIds.length ? assignedStaffIds : [null]).map((staffId) => createNotification({
      ...
      entityId: row.id,
      href: "/admin/tasks",
      metadata: { sourceType: input.entityType, sourceId: input.entityId, templateKey: template.key },
    })));
  }
  if (created.length) {
...
async function createApprovalRequest(input: {
  ...
  await ensureAdminExtensionsTables();
  const [draft] = await db.insert(approvalRequestsTable).values({
  ...
  const [row] = await db.update(approvalRequestsTable).set({ requestNo }).where(eq(approvalRequestsTable.id, draft.id)).returning();
  ...
  if (input.entityType && input.entityId) {
    await addEntityTimeline({
      ...
      body: row.title,
      actor: input.actor,
      metadata: { approvalRequestId: row.id, type: row.type, status: row.status },
    });
  }
  return row;
}
  ...
  if (input.eventDate) {
    ...
    if (!Number.isNaN(eventDate.getTime())) {
      ...
      await createNotificationOnce({
        ...
        entityId: input.entityId,
        href: erpEntityHref(input.entityType, input.entityId),
        metadata: { eventDate: input.eventDate, reminderAt: tomorrow.toISOString() },
      });
    }
  }
}
...
async function smartAlertExists(input: Pick<SmartAlertInput, "type" | "entityType" | "entityId">) {
  await ensureAdminExtensionsTables();
...
async function sendSmartAlertSideChannels(input: SmartAlertInput) {
  const telegramBody = input.telegram ?? `🔔 <b>${telegramText(input.title)}</b>\n${telegramText(input.body)}`;
  try {
    await sendTelegramMessage(telegramBody);
  } catch (err: any) {
    console.warn("smart telegram notification failed", { type: input.type, message: err?.message });
  }
  ...
  if (input.phone) {
    try {
      ...
      console.warn("smart whatsapp notification failed", { type: input.type, message: err?.message });
    }
  }
}
...
function taskProgressFromRows(rows: Array<typeof tasksTable.$inferSelect>) {
  const active = rows.filter((row) => !row.archivedAt && row.status !== "cancelled");
  if (!active.length) return { total: 0, completed: 0, percent: 0 };
  const completed = active.filter((row) => row.status === "completed").length;
  return { total: active.length, completed, percent: Math.round((completed / active.length) * 100) };
}

async function taskProgressForRelated(rows: Array<typeof tasksTable.$inferSelect>) {
...
function completedStepsForEntityStatus(entityType: string, status: string | null | undefined, total: number) {
  ...
  if (entityType === "kosha_booking") {
    if (["in_progress", "installing", "executing"].includes(value)) return Math.min(total, 3);
  ...
  if (entityType === "photography_order") {
    const order = ["registered", "editing", "ready_print", "ready_pickup", "delivered"];
    const index = order.indexOf(value);
    if (index >= 0) return Math.min(total, index + (value === "delivered" ? 1 : 0));
  }
  if (["processing", "in_progress", "preparing"].includes(value)) return Math.min(total, 2);
...
async function syncAutomaticTasksForEntityStatus(input: {
  ...
  const completedSteps = completedStepsForEntityStatus(input.entityType, input.status, rows.length);
  const changed: Array<{ id: number; from: string; to: string; title: string }> = [];
  for (const [index, task] of rows.entries()) {
    const nextStatus = completedSteps < 0
      ? "cancelled"
      : index < completedSteps
        ? "completed"
        : index === completedSteps && completedSteps > 0 && completedSteps < rows.length
          ? "in_progress"
          : task.status === "completed" && index >= completedSteps
            ? "new"
            : task.status;
    if (nextStatus !== task.status) {
      const [updated] = await db.update(tasksTable).set({ status: nextStatus, updatedAt: new Date() }).where(eq(tasksTable.id, task.id)).returning();
      changed.push({ id: task.id, from: task.status, to: nextStatus, title: task.title });
      if (nextStatus === "in_progress") {
        await Promise.all((updated.assignedStaffIds?.length ? updated.assignedStaffIds : [null]).map((staffId) => createNotification({
          type: "task_started",
          ...
          entityId: updated.id,
          href: "/admin/tasks",
          metadata: { sourceType: input.entityType, sourceId: input.entityId },
        })));
      }
    }
  }
  ...
  const latest = await db.query.tasksTable.findMany({
    where: and(eq(tasksTable.relatedType, input.entityType), eq(tasksTable.relatedId, input.entityId), sql`${tasksTable.archivedAt} is null`),
  });
...
function stockMovementKind(reason: string) {
  ...
  if (/purchase|شراء|increased|initial/.test(r)) return { type: "purchase", label: "شراء / دخول مخزن" };
  if (/deduct|sale|order|invoice|pos|خروج/.test(r)) return { type: "out", label: "خروج / بيع" };
  if (/restored|return|رجوع|reverted/.test(r)) return { type: "return", label: "رجوع للمخزون" };
  if (/maintenance|صيانة/.test(r)) return { type: "maintenance", label: "صيانة" };
  if (/damage|loss|تلف|فقد/.test(r)) return { type: "damage", label: "تلف / فقد" };
  if (/replace|استبدال/.test(r)) return { type: "replacement", label: "استبدال" };
  if (/manual|adjustment/.test(r)) return { type: "adjustment", label: "تعديل يدوي" };
  return { type: "movement", label: "حركة مخزون" };
}

async function runSmartNotificationsSweep() {
  await Promise.all([ensureAdminExtensionsTables(), ensureKoshaTables(), ensureStockTrackingTables()]);
  const now = new Date();
  ...
  const [serviceTomorrow, koshaTomorrow, overdueOrders, overdueServiceOrders, overdueKoshas, notReturnedKoshas] = await Promise.all([
    ...
    db.query.ordersTable.findMany({
      where: and(
        ...
        or(lte(ordersTable.dueDate, todayText), sql`${ordersTable.dueDate} is null and ${ordersTable.createdAt} < ${today}`),
      ),
      limit: 80,
    }),
    db.query.serviceOrdersTable.findMany({
      where: and(
        ...
        or(lte(serviceOrdersTable.dueDate, todayText), sql`${serviceOrdersTable.dueDate} is null and ${serviceOrdersTable.createdAt} < ${today}`),
      ),
      limit: 80,
    }),
    db.query.koshaBookingsTable.findMany({
      where: and(
        ...
        or(lte(koshaBookingsTable.dueDate, todayText), sql`${koshaBookingsTable.dueDate} is null and ${koshaBookingsTable.createdAt} < ${today}`),
      ),
      limit: 80,
    }),
    db.query.koshaBookingsTable.findMany({
  ...
  for (const row of overdueOrders) {
    await add({
      ...
      metadata: { remainingAmount: Number(row.remainingAmount), dueDate: row.dueDate },
    });
  }
...
async function handleKoshas(req: NextRequest, parts: string[]) {
  ...
  if (method === "POST" && parts[1] && parts[2] === "bookings") {
    ...
    }).catch((err) => console.error("auto kosha tasks failed", { bookingId: booking.id, message: err?.message }));
    void addEntityTimeline({
      entityType: "kosha_booking",
      entityId: booking.id,
      type: "created",
      ...
      body: `${customerName} - ${packageDetails?.name ?? kosha.name}`,
      metadata: { trackingCode, packageId: packageDetails?.id ?? null, koshaId: kosha.id },
    }).catch((err) => console.error("kosha timeline failed", { bookingId: booking.id, message: err?.message }));
    ...
    }).catch((err) => console.error("kosha smart notifications failed", { bookingId: booking.id, message: err?.message }));
    void notifyTelegramKoshaBooking({
      ...
      items: packageDetails
        ? [{ productName: packageDetails.name, quantity: 1, unitPrice: 0, total: 0 }]
        : [
            { productName: kosha.name, quantity: 1, unitPrice: 0, total: 0 },
            ...[...addonDetails, ...welcomeBoardDetails, ...accessoryDetails].map((item) => ({ productName: item.name, quantity: 1, unitPrice: 0, total: 0 })),
          ],
    });
...
async function handleServiceOrders(req: NextRequest, parts: string[]) {
  ...
  if (method === "POST" && parts.length === 1) {
    ...
    }).catch((err) => console.error("auto service tasks failed", { orderId: order.id, message: err?.message }));
    void addEntityTimeline({
      entityType: "service_order",
      entityId: order.id,
      type: "created",
      ...
      body: `${order.customerName} - ${service?.nameAr ?? service?.name ?? "خدمة"}`,
      metadata: { trackingCode: order.trackingCode, serviceId: order.serviceId, serviceType: service.type },
    }).catch((err) => console.error("service timeline failed", { orderId: order.id, message: err?.message }));
    ...
    void createSmartBookingNotifications({
      ...
      eventDate: order.eventDate,
      remainingAmount: order.remainingAmount,
    }).catch((err) => console.error("service smart notifications failed", { orderId: order.id, message: err?.message }));
...
function iso(value: any): string | null {
  return value?.toISOString?.() ?? (value ? new Date(value).toISOString() : null);
}
...
async function handleAdmin(req: NextRequest, parts: string[]) {
  ...
  if (section === "approvals") {
    ...
    if (method === "GET") {
      ...
      if (q) filters.push(or(ilike(approvalRequestsTable.title, `%${q}%`), ilike(approvalRequestsTable.description, `%${q}%`), ilike(approvalRequestsTable.requestNo, `%${q}%`)));
    ...
    if (method === "POST" && !parts[2]) {
      ...
      const row = await createApprovalRequest({
        ...
        oldValues: typeof b?.oldValues === "object" && b.oldValues ? b.oldValues : {},
        newValues: typeof b?.newValues === "object" && b.newValues ? b.newValues : {},
        actor: erpActorFromAdmin(auth),
      });
      void logAdminActivity(req, "approval_request_created", "approval_request", row.id, { type: row.type, title });
      return json(formatApprovalRequest(row), 201);
    }
    if (method === "PATCH" && parts[2]) {
      const id = int(parts[2]);
      if (!id) return error("معرف غير صحيح", 400);
      if (!(auth.role === "admin" || auth.role === "manager")) return error("اعتماد الطلبات متاح للمدير فقط", 403);
      const b = await body(req);
      const status = String(b?.status ?? b?.action ?? "").toLowerCase();
      const next = status === "approved" || status === "approve" ? "approved" : status === "rejected" || status === "reject" ? "rejected" : "";
      if (!next) return error("اختر موافقة أو رفض", 400);
      const existing = await db.query.approvalRequestsTable.findFirst({ where: eq(approvalRequestsTable.id, id) });
      if (!existing) return error("طلب الموافقة غير موجود", 404);
      if (existing.status !== "pending") return error("تمت مراجعة هذا الطلب مسبقاً", 409);
      const [row] = await db.update(approvalRequestsTable).set({
        status: next,
        reviewedBy: auth.id,
  ...
  if (section === "smart-notifications") {
    const auth = await requireAnyPermission(req, ["dashboard", "tasks", "orders"]);
    if (isResponse(auth)) return auth;
    if (method === "GET" || method === "POST") {
      const summary = await runSmartNotificationsSweep();
      void logAdminActivity(req, "smart_notifications_sweep", "notification", undefined, summary);
  ...
  if (section === "documents") {
    ...
    if (method === "GET") {
      ...
      if (entityType) filters.push(eq(entityDocumentsTable.entityType, entityType));
    ...
    if (method === "POST") {
      ...
      if (!entityType || !entityId || !fileUrl) return error("الكيان والملف مطلوبان", 400);
      const [row] = await db.insert(entityDocumentsTable).values({
        entityType,
        entityId,
      ...
      void addEntityTimeline({ entityType, entityId, type: "document_added", title: "تمت إضافة مستند", body: row.title, actor: erpActorFromAdmin(auth), metadata: { documentId: row.id } });
      return json(formatEntityDocument(row), 201);
    }
    if (method === "DELETE" && parts[2]) {
      const id = int(parts[2]);
      if (!id) return error("معرف غير صحيح", 400);
      const existing = await db.query.entityDocumentsTable.findFirst({ where: eq(entityDocumentsTable.id, id) });
      ...
      const approval = await createApprovalRequest({
        ...
        oldValues: { documentId: existing.id, title: existing.title, fileUrl: existing.fileUrl },
        actor: erpActorFromAdmin(auth),
      });
      return json({ message: "تم إرسال طلب حذف المستند للموافقة", approval: formatApprovalRequest(approval) }, 202);
    }
  }
  ...
  if (section === "entity-timeline") {
    ...
    const rows = await db.query.entityTimelineTable.findMany({
      where: and(eq(entityTimelineTable.entityType, entityType), eq(entityTimelineTable.entityId, entityId)),
  ...
  if (section === "activity-timeline") {
    const auth = await requireAnyPermission(req, ["dashboard", "orders", "bookings", "tasks", "staff"]);
    if (isResponse(auth)) return auth;
    ...
    if (q) {
      timelineFilters.push(or(ilike(entityTimelineTable.title, `%${q}%`), ilike(entityTimelineTable.body, `%${q}%`), ilike(entityTimelineTable.actorName, `%${q}%`)));
      activityFilters.push(or(ilike(adminActivityLogsTable.action, `%${q}%`), ilike(adminActivityLogsTable.userName, `%${q}%`), sql`${adminActivityLogsTable.metadata}::text ilike ${`%${q}%`}`));
    }
    ...
    const data = [
      ...timelineRows.map((row) => ({ ...formatTimelineRow(row), source: "entity" })),
      ...activityRows.map((row) => ({
        id: row.id,
        type: row.action,
        title: row.action,
        body: row.entityType ? `${row.entityType}${row.entityId ? ` #${row.entityId}` : ""}` : JSON.stringify(row.metadata ?? {}),
  ...
  if (section === "financial-timeline") {
    ...
    const filters: any[] = [];
    if (entityType) filters.push(eq(financialTransactionsTable.sourceType, entityType));
    if (entityId) filters.push(eq(financialTransactionsTable.sourceId, entityId));
    ...
    const auditRows = rows.length
      ? await db.query.financialAuditLogsTable.findMany({
        where: inArray(financialAuditLogsTable.transactionId, rows.map((row) => row.id)),
    ...
    return json({ data: rows.map((row) => ({
      ...
      profitImpact: row.direction === "revenue" ? Number(row.amount) : -Number(row.amount),
      audit: auditRows.filter((audit) => audit.transactionId === row.id).map((audit) => ({
        id: audit.id,
        action: audit.action,
  ...
  if (section === "inventory-timeline") {
    ...
    if (isResponse(auth)) return auth;
    await ensureStockTrackingTables();
    const productId = optionalPositiveId(req.nextUrl.searchParams.get("productId"));
    const rows = await db.query.stockMovementsTable.findMany({
      where: productId ? or(eq(stockMovementsTable.productId, productId), eq(stockMovementsTable.stockSourceProductId, productId)) : undefined,
      orderBy: [desc(stockMovementsTable.createdAt)],
      limit: 250,
    });
    const productIds = Array.from(new Set(rows.flatMap((row) => [row.productId, row.stockSourceProductId]).filter((id): id is number => Boolean(id))));
    const productRows = productIds.length ? await db.query.productsTable.findMany({ where: inArray(productsTable.id, productIds) }) : [];
    const productNameById = new Map(productRows.map((row) => [row.id, row.nameAr || row.name]));
    return json({ data: rows.map((row) => {
      const kind = stockMovementKind(row.reason);
      return {
        id: row.id,
        productId: row.productId,
        productName: row.productId ? productNameById.get(row.productId) ?? "" : "",
        stockSourceProductId: row.stockSourceProductId,
        stockSourceProductName: row.stockSourceProductId ? productNameById.get(row.stockSourceProductId) ?? "" : "",
  ...
  if (section === "live-operations") {
    const auth = await requireAnyPermission(req, ["dashboard", "orders", "tasks", "photography"]);
    if (isResponse(auth)) return auth;
    await ensureAdminExtensionsTables();
    await ensureKoshaTables();
    await ensurePhotographyStaffTables();
    ...
    const [koshas, photos, tasks] = await Promise.all([
      db.query.koshaBookingsTable.findMany({
        where: and(sql`${koshaBookingsTable.archivedAt} is null`, sql`${koshaBookingsTable.status} not in ('completed','cancelled')`),
        orderBy: [asc(koshaBookingsTable.eventDate), desc(koshaBookingsTable.createdAt)],
        limit: 50,
      }),
      db.query.photographyEventsTable.findMany({
        where: sql`${photographyEventsTable.status} <> 'archived'`,
        orderBy: [asc(photographyEventsTable.eventDate), desc(photographyEventsTable.id)],
        limit: 50,
      }),
      db.query.tasksTable.findMany({
        where: and(sql`${tasksTable.archivedAt} is null`, sql`${tasksTable.status} not in ('completed','cancelled')`),
  ...
  if (section === "smart-search") {
    const auth = await requireAnyPermission(req, ["dashboard", "orders", "customers", "products", "accounting"]);
    if (isResponse(auth)) return auth;
    const q = req.nextUrl.searchParams.get("q")?.trim() ?? "";
    if (q.length < 2) return json({ data: [] });
    const likeQ = `%${q}%`;
    const [customers, orders, products, invoices, documents] = await Promise.all([
      db.query.customersTable.findMany({ where: or(ilike(customersTable.fullName, likeQ), ilike(customersTable.phone, likeQ)), limit: 8 }),
      db.query.ordersTable.findMany({ where: or(ilike(ordersTable.customerName, likeQ), ilike(ordersTable.customerPhone, likeQ), ilike(ordersTable.trackingCode, likeQ)), limit: 8 }),
      db.query.productsTable.findMany({ where: or(ilike(productsTable.name, likeQ), ilike(productsTable.nameAr, likeQ), ilike(productsTable.barcode, likeQ)), limit: 8 }),
      db.query.salesInvoicesTable.findMany({ where: or(ilike(salesInvoicesTable.invoiceNo, likeQ), ilike(salesInvoicesTable.customerName, likeQ), ilike(salesInvoicesTable.customerPhone, likeQ)), limit: 8 }),
      db.query.entityDocumentsTable.findMany({ where: and(sql`${entityDocumentsTable.archivedAt} is null`, or(ilike(entityDocumentsTable.title, likeQ), ilike(entityDocumentsTable.fileName, likeQ))), limit: 8 }),
    ]);
    return json({ data: [
      ...customers.map((row) => ({ type: "customer", id: row.id, title: row.fullName || row.phone, subtitle: row.phone, href: "/admin/customers" })),
      ...orders.map((row) => ({ type: "order", id: row.id, title: row.trackingCode, subtitle: row.customerName, href: "/admin/orders" })),
      ...products.map((row) => ({ type: "product", id: row.id, title: row.nameAr || row.name, subtitle: row.barcode ?? "", href: "/admin/products" })),
      ...invoices.map((row) => ({ type: "invoice", id: row.id, title: row.invoiceNo, subtitle: row.customerName, href: "/admin/sales" })),
      ...documents.map((row) => ({ type: "document", id: row.id, title: row.title, subtitle: row.entityType, href: row.fileUrl })),
    ] });
  }
  ...
  if (section === "business-analytics") {
    ...
    if (isResponse(auth)) return auth;
    const [serviceRows, productRows, customerRows, staffRows, monthRows] = await Promise.all([
      db.execute(sql`select coalesce(s.type, 'store') as label, coalesce(sum(so.total_amount::numeric),0) as total, count(*)::int as count from service_orders so left join services s on s.id=so.service_id group by coalesce(s.type, 'store') order by total desc limit 8`),
      db.execute(sql`select oi.product_name as label, sum(oi.quantity)::int as count, coalesce(sum(oi.price::numeric * oi.quantity),0) as total from order_items oi group by oi.product_name order by count desc limit 8`),
      db.execute(sql`select customer_name as label, count(*)::int as count, coalesce(sum(total::numeric),0) as total from orders group by customer_name order by count desc limit 8`),
      db.execute(sql`select coalesce(nullif(requested_by_name, ''), 'النظام') as label, count(*)::int as count from financial_transactions group by coalesce(nullif(requested_by_name, ''), 'النظام') order by count desc limit 8`),
      db.execute(sql`select to_char(created_at, 'YYYY-MM') as label, count(*)::int as count from orders group by to_char(created_at, 'YYYY-MM') order by label desc limit 12`),
    ]);
    const rows = (result: any) => (result.rows ?? []).map((row: any) => ({ ...row, total: Number(row.total ?? 0), count: Number(row.count ?? 0) }));
    return json({
  ...
  if (section === "warehouse-transfers") {
    ...
    if (method === "POST") {
      const b = await body(req);
      const productId = optionalPositiveId(b?.productId);
      const product = productId ? await db.query.productsTable.findFirst({ where: eq(productsTable.id, productId) }) : null;
      const qty = Math.max(1, Number.parseInt(String(b?.quantity ?? "1"), 10) || 1);
      const [draft] = await db.insert(warehouseTransfersTable).values({
        transferNo: `WTR-TMP-${randomUUID()}`,
        productId,
        productName: textFallback(product?.nameAr, product?.name, b?.productName, "مادة"),
      ...
      const [row] = await db.update(warehouseTransfersTable).set({ transferNo }).where(eq(warehouseTransfersTable.id, draft.id)).returning();
      await createApprovalRequest({ type: "warehouse_transfer", title: `تحويل مخزني ${transferNo}`, entityType: "warehouse_transfer", entityId: row.id, newValues: { productId, qty }, actor: erpActorFromAdmin(auth) });
      return json(formatWarehouseTransfer(row), 201);
    }
    if (method === "PATCH" && parts[2]) {
      const id = int(parts[2]);
      if (!id) return error("معرف غير صحيح", 400);
      if (!(auth.role === "admin" || auth.role === "manager")) return error("اعتماد التحويل متاح للمدير فقط", 403);
      const b = await body(req);
  ...
  if (section === "purchase-comparison") {
    ...
    const productId = optionalPositiveId(req.nextUrl.searchParams.get("productId"));
    const filters: any[] = [];
    if (productId) filters.push(eq(purchaseInvoiceItemsTable.productId, productId));
    ...
    const rows = await db
      ...
      .orderBy(desc(purchaseInvoicesTable.date))
      .limit(80);
    const costs = rows.map((row) => Number(row.costPrice ?? 0)).filter((n) => Number.isFinite(n));
    return json({ data: rows, summary: { lastPrice: costs[0] ?? 0, bestPrice: costs.length ? Math.min(...costs) : 0, bestSupplier: rows.find((row) => Number(row.costPrice) === Math.min(...costs))?.supplierName ?? "" } });
  }
  ...
  if (section === "maintenance-scheduler") {
    ...
    const profiles = await db.query.assetProfilesTable.findMany({ limit: 500 });
    const profileByProduct = new Map(profiles.map((row) => [row.productId, row]));
    const due = products.map((product) => {
      const profile = profileByProduct.get(product.id);
      const usageCount = Number(profile?.usageCount ?? 0);
      const every = Number(profile?.maintenanceEveryUses ?? 50) || 50;
      const nextAt = Math.max(every, Math.ceil((usageCount + 1) / every) * every);
      return { productId: product.id, name: product.nameAr || product.name, usageCount, maintenanceEveryUses: every, nextMaintenanceAt: nextAt, due: usageCount > 0 && usageCount >= nextAt - every && usageCount % every === 0 };
  ...
  if (section === "disaster-recovery") {
    ...
    if (method === "GET") {
      ...
      if (id && parts[3] === "download") {
        const snapshot = await db.query.disasterRecoverySnapshotsTable.findFirst({ where: eq(disasterRecoverySnapshotsTable.id, id) });
      ...
      const rows = await db.query.disasterRecoverySnapshotsTable.findMany({ orderBy: [desc(disasterRecoverySnapshotsTable.createdAt)], limit: 80 });
      return json({ data: rows.map((row) => ({ ...row, createdAt: iso(row.createdAt) })) });
    }
    if (method === "POST" && parts[2] && parts[3] === "restore") {
      const id = optionalPositiveId(parts[2]);
      if (!id) return error("معرف اللقطة غير صحيح", 400);
      const b = await body(req);
      if (b?.confirm !== "AJN-RESTORE-CONFIRMED") return error("اكتب تأكيد الاسترجاع أولاً", 400);
      const snapshot = await db.query.disasterRecoverySnapshotsTable.findFirst({ where: eq(disasterRecoverySnapshotsTable.id, id) });
      if (!snapshot) return error("اللقطة غير موجودة", 404);
      let payload: any = (snapshot.summary as any)?.inlineBackup ?? null;
      if (snapshot.fileUrl) {
      ...
      const [row] = await db.update(disasterRecoverySnapshotsTable).set({
        status: "restored",
        summary,
      }).where(eq(disasterRecoverySnapshotsTable.id, id)).returning();
      void logAdminActivity(req, "disaster_recovery_snapshot_restored", "disaster_recovery_snapshot", id, { snapshotNo: snapshot.snapshotNo, report });
      return json({ ...row, createdAt: iso(row.createdAt), report });
    }
    if (method === "POST") {
      const [draft] = await db.insert(disasterRecoverySnapshotsTable).values({
      ...
      const [row] = await db.update(disasterRecoverySnapshotsTable).set({
        ...
        fileUrl,
        summary,
      }).where(eq(disasterRecoverySnapshotsTable.id, draft.id)).returning();
      void logAdminActivity(req, "disaster_recovery_snapshot_created", "disaster_recovery_snapshot", row.id, { snapshotNo, counts });
    ...
    if ((method === "PATCH" || method === "DELETE") && parts[2]) {
      ...
      if (row.relatedType && row.relatedId) {
        void addEntityTimeline({
          ...
          actor: erpActorFromAdmin(auth),
          metadata: { taskId: row.id, oldStatus: existing.status, newStatus: row.status, fields: Object.keys(update) },
        });
      }
  ...
  if (section === "dashboard" && method === "GET") {
    ...
    runAfter("smart-notifications-sweep", runSmartNotificationsSweep);
    const alerts = [
  ...
  if (section === "service-orders") {
    ...
    if (method === "POST" && !parts[2]) {
      ...
      void createAutomaticTasksForEntity({
        ...
        eventDate: order.eventDate,
        createdBy: auth.id,
      }).catch((err) => console.error("auto admin service tasks failed", { orderId: order.id, message: err?.message }));
      void addEntityTimeline({
        entityType: "service_order",
        entityId: order.id,
        type: "created",
        ...
        actor: erpActorFromAdmin(auth),
        metadata: { trackingCode: order.trackingCode, serviceId: order.serviceId, serviceType: service.type },
      }).catch((err) => console.error("admin service timeline failed", { orderId: order.id, message: err?.message }));
      void createCustomerNotificationByPhone(order.phone, {
        type: "booking_created",
        title: "تم إنشاء الحجز",
        body: `رمز التتبع ${order.trackingCode ?? ""}`,
      ...
      void createSmartBookingNotifications({
        ...
        eventDate: order.eventDate,
        remainingAmount: order.remainingAmount,
      }).catch((err) => console.error("admin service smart notifications failed", { orderId: order.id, message: err?.message }));
    ...
    if (method === "PATCH" && parts[2]) {
      ...
      if (typeof update.status === "string" && update.status && update.status !== prev?.status) {
        ...
        void syncAutomaticTasksForEntityStatus({
          ...
          status: row.status,
          actor: erpActorFromAdmin(auth),
        }).catch((err) => console.error("service order task status sync failed", { orderId: row.id, message: err?.message }));
        void addEntityTimeline({
          entityType: "service_order",
          entityId: row.id,
          type: "status_changed",
    ...
    if (method === "DELETE" && parts[2]) {
      ...
      void syncAutomaticTasksForEntityStatus({
        ...
        status: "cancelled",
        actor: erpActorFromAdmin(auth),
      }).catch((err) => console.error("service order delete task sync failed", { orderId: id, message: err?.message }));
      void addEntityTimeline({
        entityType: "service_order",
        entityId: id,
        type: "cancelled",
...
async function handlePurchaseInvoices(req: NextRequest, parts: string[], section: string | undefined) {
  ...
  if (method === "GET" && id) {
    ...
    const pids = items.map((it: any) => it.productId).filter(Boolean) as number[];
    const prods = pids.length ? await db.query.productsTable.findMany({ where: inArray(productsTable.id, pids) }) : [];
    const imgMap = new Map(prods.map((p: any) => [p.id, Array.isArray(p.images) && p.images.length ? p.images[0] : null]));
    return json({ ...inv, items: items.map((it: any) => ({ ...it, image: it.productId ? imgMap.get(it.productId) ?? null : null })) });
  }
  ...
  if (method === "PUT" && id) {
    const existing = await db.query.purchaseInvoicesTable.findFirst({ where: eq(purchaseInvoicesTable.id, id) });
    if (!existing || existing.status === "deleted") return error("الفاتورة غير موجودة", 404);
    const b = await body(req);
    const a = actor(auth);
    const newItems = purchaseInvoiceItems(b?.items);
    if (newItems.length === 0) return error("أضف صنفاً واحداً على الأقل إلى فاتورة الشراء", 400);

    // Reverse the stock added by the old items, then remove them.
    const oldItems = await db.select().from(purchaseInvoiceItemsTable).where(eq(purchaseInvoiceItemsTable.invoiceId, id));
    for (const oi of oldItems as any[]) {
      if (oi.productId && Number(oi.quantity) > 0) {
        await adjustProductStock(Number(oi.productId), -Number(oi.quantity), { reason: "purchase_invoice_edit_reversal", relatedType: "purchase_invoice", relatedId: id, createdBy: a.id, createdByName: a.name });
      }
    }
    await db.delete(purchaseInvoiceItemsTable).where(eq(purchaseInvoiceItemsTable.invoiceId, id));

    const subtotal = newItems.reduce((sum, item) => sum + item.quantity * item.costPrice, 0);
    const discountAmount = parseFloat(b.discountAmount ?? "0") || 0;
    const taxAmount = parseFloat(b.taxAmount ?? "0") || 0;
    const shippingCost = parseFloat(b.shippingCost ?? "0") || 0;
    const total = Math.max(subtotal - discountAmount + taxAmount + shippingCost, 0);
    ...
    for (const item of newItems) {
      ...
      if (!productId) {
        const existingProduct = item.barcode
          ? await db.query.productsTable.findFirst({ where: eq(productsTable.barcode, item.barcode) })
          : await db.query.productsTable.findFirst({ where: or(eq(productsTable.nameAr, item.productName), eq(productsTable.name, item.productName)) });
        if (existingProduct) productId = existingProduct.id;
        else {
          const [createdProduct] = await db.insert(productsTable).values({ name: item.productName, nameAr: item.productName, price: String(item.salePrice > 0 ? item.salePrice : item.costPrice), costPrice: String(item.costPrice), stock: 0, barcode: item.barcode || null, category: "purchases", images: [], colors: [], isActive: true } as any).returning();
          productId = createdProduct.id;
        }
      }
      processedItems.push({ ...item, productId });
    ...
    await db.insert(purchaseInvoiceItemsTable).values(
      processedItems.map((item: any) => ({ invoiceId: id, productId: item.productId ?? null, productName: item.productName ?? "", barcode: item.barcode, quantity: String(item.quantity), costPrice: String(item.costPrice), salePrice: String(item.salePrice), discount: String(item.discount), total: String(item.total) })),
    );
    for (const item of processedItems) {
      if (item.productId && item.quantity > 0) {
        await adjustProductStock(Number(item.productId), Number(item.quantity), { reason: "purchase_invoice_edit_applied", relatedType: "purchase_invoice", relatedId: id, createdBy: a.id, createdByName: a.name });
        const updateVals: any = { costPrice: String(item.costPrice) };
    ...
    const final = await db.query.purchaseInvoicesTable.findFirst({ where: eq(purchaseInvoicesTable.id, id) });
    const finalItems = await db.select().from(purchaseInvoiceItemsTable).where(eq(purchaseInvoiceItemsTable.invoiceId, id));
    if (final) {
      await syncSourcePaymentTarget({
        sourceType: "purchase_invoice", sourceId: final.id, sourceEvent: "payment",
        targetAmount: Number(final.paidAmount), normalDirection: "expense",
        transactionDate: final.date, department: "store", transactionType: "purchase_invoice",
        description: `فاتورة شراء ${final.invoiceNo}`, paymentMethod: (final.paymentMethod as any) || "cash", notes: final.notes,
      }, financialActor(auth));
    }
    void logAdminActivity(req, "purchase_invoice_updated", "purchase_invoice", id, { invoiceNo: final?.invoiceNo, itemCount: finalItems.length });
    return json({ ...final, items: finalItems, invoice: final });
  }

  if (method === "DELETE" && id) {
    const current = await db.query.purchaseInvoicesTable.findFirst({ where: eq(purchaseInvoicesTable.id, id) });
    if (!current) return error("الفاتورة غير موجودة", 404);
    if (current.status === "deleted") return json({ message: "الفاتورة محذوفة مسبقاً" });
    const a = actor(auth);
    // Reverse the stock this invoice added, so deletion keeps inventory correct.
    const items = await db.select().from(purchaseInvoiceItemsTable).where(eq(purchaseInvoiceItemsTable.invoiceId, id));
    for (const it of items as any[]) {
      if (it.productId && Number(it.quantity) > 0) {
        await adjustProductStock(Number(it.productId), -Number(it.quantity), { reason: "purchase_invoice_deleted_reversal", relatedType: "purchase_invoice", relatedId: id, createdBy: a.id, createdByName: a.name });
      }
    }
    ...
    void logAdminActivity(req, "purchase_invoice_deleted", "purchase_invoice", id, { invoiceNo: current.invoiceNo, itemCount: items.length });
    return json({ message: "تم الحذف" });
  }
  ...
  if (resource === "events") {
    ...
    if (method === "POST" && !ref) {
      ...
      void createAutomaticTasksForEntity({
        ...
        createdBy: auth.id,
      }).catch((err) => console.error("auto photography tasks failed", { eventId: row.id, message: err?.message }));
      void addEntityTimeline({
        entityType: "photography_event",
        entityId: row.id,
        type: "created",
        ...
        actor: erpActorFromAdmin(auth),
        metadata: { assignedStaffId: assigned.id, clientToken: row.clientToken },
      }).catch((err) => console.error("photography event timeline failed", { eventId: row.id, message: err?.message }));
    ...
    if (method === "POST" && ref && action === "orders") {
      ...
      void createAutomaticTasksForEntity({
        ...
        createdBy: auth.id,
      }).catch((err) => console.error("auto photography order tasks failed", { orderId: order.id, message: err?.message }));
      void addEntityTimeline({
        entityType: "photography_order",
        entityId: order.id,
        type: "created",
      ...
      }).catch((err) => console.error("photography order timeline failed", { orderId: order.id, message: err?.message }));
      void createSmartBookingNotifications({
        entityType: "photography_order",
        entityId: order.id,
        title: orderNo,
  ...
  if (resource === "orders") {
    ...
    if (method === "PATCH" && !action) {
      ...
      void addEntityTimeline({
        ...
        actor: { id: auth.id, name: photographyStaffName(auth) },
        metadata: { fields: Object.keys(data), oldValues: order, newValues: updated },
      });
...
function optionalPositiveId(value: unknown): number | null {
  ...
  const normalized = typeof value === "string" ? normalizePhoneDigits(value) : value;
...
function normalizeFinanceDepartment(value: unknown): string {
  ...
  const ascii = lower.replace(/[^a-z0-9_-]+/g, "_").replace(/^_+|_+$/g, "").slice(0, 40);
...
const voucherMethodSchema = z.enum(["cash", "transfer", "pos", "card"], { error: "طريقة الدفع غير صحيحة" })
  .transform((value) => normMethod(value));

const financeDepartmentSchema = z.preprocess(
  normalizeFinanceDepartment,
  z.string().min(1).max(40).regex(/^[a-zA-Z0-9_-]+$/, "رمز القسم غير صحيح"),
);
...
const receiptVoucherMutationSchema = z.object({
  ...
  orderId: z.preprocess(optionalPositiveId, z.number().int().positive().nullable()).optional().default(null),
  bookingId: z.preprocess(optionalPositiveId, z.number().int().positive().nullable()).optional().default(null),
...
function accountingMutationError(err: unknown, fallback: string): { message: string; status: number } {
  ...
  if (/permission denied|row-level security|RLS|policy/i.test(message)) {
    return { message: "صلاحيات قاعدة البيانات لا تسمح بإكمال الحفظ. تأكد من Service Role/RLS لجداول الحسابات.", status: 403 };
  }
  if (/duplicate key|unique constraint|violates unique/i.test(message)) {
    return { message: "رقم السند تكرر في قاعدة البيانات. حاول الحفظ مرة أخرى.", status: 409 };
  }
  if (/foreign key|violates foreign key/i.test(message)) {
    return { message: "يوجد مرجع غير صحيح مرتبط بالسند. حدّث الصفحة وحاول مرة أخرى.", status: 400 };
  }
  ...
  if (section === "receipt-vouchers") {
    ...
    if (method === "POST") {
      ...
      try {
        ...
        console.error("receipt voucher save failed", { message: err?.message, code: err?.code, detail: err?.detail });
        const safe = accountingMutationError(err, "تعذر حفظ سند القبض. تحقق من البيانات أو صلاحيات قاعدة البيانات.");
  ...
  if (section === "payment-vouchers") {
    ...
    if (method === "POST") {
      ...
      try {
        ...
        console.error("payment voucher save failed", { message: err?.message, code: err?.code, detail: err?.detail });
        const safe = accountingMutationError(err, "تعذر حفظ سند الصرف. تحقق من البيانات أو صلاحيات قاعدة البيانات.");
        return error(safe.message, safe.status);
      }
    }
    if (method === "DELETE" && parts[2]) {
...
const BACKUP_IMPORT_ORDER: BackupEntity[] = [
  ...
  "kosha_images",
  "kosha_packages",
  "kosha_package_components",
  "kosha_addons",
  "kosha_welcome_boards",
  "kosha_accessories",
  "kosha_provinces",
  "kosha_bookings",
  "kosha_booking_events",
  "photography_events",
  "photography_orders",
  "photography_order_events",
  "photography_payment_requests",
  "notifications",
  "tasks",
  "task_comments",
  "approval_requests",
...
function collectBackupMedia(rows: any[]) {
  ...
  const visit = (value: unknown) => {
    ...
    if (typeof value === "string") {
      if (/^https?:\/\//i.test(value) || value.startsWith("/api/media/")) urls.add(value);
      return;
    }
    if (Array.isArray(value)) {
...
async function buildBackupPayload(): Promise<BackupPayload> {
  ...
  for (const [name, table] of Object.entries(BACKUP_ENTITIES) as [string, any][]) {
    try {
      ...
      console.warn("backup entity export failed", { name, message: err?.message });
      data[name] = [];
    }
  }
  ...
  for (const name of BACKUP_IMPORT_ORDER) {
    ...
    for (const row of rows) {
      try {
        ...
        stats.errors++;
        console.warn("backup entity import row failed", { name, message: err?.message });
      }
    }
    report[name] = stats;
  }
