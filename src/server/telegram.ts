...
const DEFAULT_EVENTS = Object.fromEntries(
  TELEGRAM_EVENT_KEYS.map((key) => [key, true]),
...
function safeError(value: unknown) {
  const message =
    value instanceof Error ? value.message : String(value ?? "خطأ غير معروف");
...
export function telegramEnvironmentStatus() {
  ...
  return {
    botTokenConfigured: Boolean(config.token),
...
export async function getTelegramSettings(): Promise<TelegramSettings> {
  ...
  const parsed = telegramSettingsSchema.safeParse(
    row?.value ?? DEFAULT_TELEGRAM_SETTINGS,
  );
...
async function eventEnabled(event: TelegramEventKey) {
  try {
    ...
    console.warn("telegram settings load failed", {
...
async function telegramRequest(
  ...
  if (!configured)
    return {
      ok: false,
      error: "إعدادات Telegram غير مكتملة في Environment Variables",
    };
  ...
  try {
    ...
    if (!response.ok || !payload?.ok) {
      ...
      console.warn("telegram request failed", {
        method,
...
function paymentLabel(value: unknown) {
  return (
    ...
    )[String(value ?? "").toLowerCase()] ?? String(value || "غير محدد")
  );
}
...
function messageBlock(title: string, rows: string[], href?: string) {
  return [
    ...
    ...rows.filter(Boolean),
    href ? "" : "",
    href ? `<a href="${escapeHtml(href)}">فتح داخل لوحة الإدارة</a>` : "",
  ]
    .filter(
      (row, index, all) => row !== "" || (index > 0 && all[index - 1] !== ""),
    )
    .join("\n");
}
...
function createPdf(
  ...
  return new Promise((resolve, reject) => {
    ...
    doc
      .fillColor("#111111")
      .fontSize(22)
      .text(title, 215, 42, { width: 280, ...rtlOptions });
    if (reference)
      doc
        .fontSize(15)
        .text(reference, 42, 48, { width: 180, align: "left", features: [] });
    doc.y = 82;
    doc
      .moveDown(0.4)
      .strokeColor("#D4B15A")
    ...
    for (const [label, value] of rows) {
      ...
      doc
        .fillColor("#444444")
        .text(label, 370, rowY, { width: 183, ...rtlOptions });
      const cleanValue = value || "-";
      doc.fillColor("#222222").text(cleanValue, 42, rowY, {
        width: 305,
        align: "right",
        features: hasArabic(cleanValue) ? ["rtla"] : [],
      });
      doc.y = rowY + 25;
    }
    if (items.length) {
      ...
      for (const item of items) {
        ...
        doc
          .fillColor("#222222")
          .fontSize(10)
          .text(name, 255, itemY, { width: 298, ...rtlOptions });
        doc.text(String(quantity), 180, itemY, {
          width: 55,
          align: "center",
          features: [],
        });
    ...
    if (qr) {
      ...
      doc
        .moveDown(0.8)
        .image(qr, 246, doc.y, { fit: [105, 105], align: "center" });
      doc.y += 112;
      doc
        .fillColor("#333333")
        .fontSize(9)
        .text("امسح الرمز لتتبع الطلب", {
          align: "center",
          features: ["rtla"],
        });
    }
    doc
      ...
      .fillColor("#777777")
      .fontSize(9)
      .text(`AJN - ${dateTime().date} ${dateTime().time}`, { align: "center" });
    doc.end();
  });
}
...
async function savePdfToStorage(
  ...
  try {
    const response = await fetch(
      `${storageUrl}/storage/v1/object/${bucket}/${objectPath}`,
      {
        method: "POST",
        headers: {
          apikey: serviceKey,
    ...
    if (!response.ok) {
      console.warn("order PDF storage upload failed", {
        folder,
        status: response.status,
      });
      return null;
...
export function generateInvoicePdf(invoice: TelegramInvoiceInput) {
  return createPdf(
    ...
    [
      ...
      [
        "الهاتف",
        invoice.customerPhone ? formatIraqiPhone(invoice.customerPhone) : "-",
      ],
...
export function generateOrderPdf(order: TelegramOrderInput) {
  return createPdf(
    order.kind === "store" ? "طلب متجر" : "حجز خدمة",
    [
      ["رقم الطلب", order.reference || `#${order.id}`],
      [
        "الخدمة",
        order.serviceName || (order.kind === "store" ? "المتجر" : "-"),
      ],
...
export async function notifyTelegramInvoice(
  invoice: TelegramInvoiceInput,
): Promise<void> {
  ...
  try {
    ...
    console.warn("invoice PDF generation failed", {
  ...
  try {
    ...
    console.warn("telegram invoice PDF generation failed", {
      invoiceId: invoice.id,
      message: safeError(error),
    });
  }
...
export async function notifyTelegramOrder(
  ...
  const event: TelegramEventKey =
    order.kind === "store" ? "storeOrder" : "serviceBooking";
  let pdf: Buffer | null = null;
  try {
    pdf = await generateOrderPdf(order);
    await savePdfToStorage(
      pdf,
      `documents/${order.kind}-orders`,
      `order-${order.reference || order.id}.pdf`,
    );
  } catch (error) {
    console.warn("order PDF generation failed", {
      orderId: order.id,
      kind: order.kind,
      message: safeError(error),
    });
  ...
  const href = adminLink(
    order.kind === "store"
      ...
      : `/admin/orders?serviceOrder=${order.id}`,
  );
  await sendTelegramMessage(
    messageBlock(
      ...
      [
        line(
          order.kind === "store" ? "رقم الطلب" : "رقم الحجز",
          order.reference || `#${order.id}`,
        ),
  ...
  if (pdf)
    await sendTelegramDocument(
      pdf,
      `order-${order.reference || order.id}.pdf`,
      `${order.kind === "store" ? "طلب" : "حجز"} ${order.reference || order.id}`,
    );
}

export async function notifyTelegramKoshaBooking(
  booking: TelegramKoshaBookingInput,
): Promise<void> {
  let pdf: Buffer | null = null;
  try {
    ...
    await savePdfToStorage(
      pdf,
      "documents/kosha-bookings",
      `kosha-booking-${booking.reference || booking.id}.pdf`,
    );
  } catch (error) {
    console.warn("kosha booking PDF generation failed", {
  ...
  if (pdf)
    await sendTelegramDocument(
      pdf,
      `kosha-booking-${booking.reference || booking.id}.pdf`,
      `حجز كوشة ${booking.reference || booking.id}`,
    );
}

export async function notifyTelegramOrderEdited(
  input: TelegramOrderEditInput,
): Promise<void> {
  const onlyStatus =
    input.changedFields?.length === 1 && input.changedFields[0] === "status";
  if (!(await eventEnabled(onlyStatus ? "statusChanged" : "orderEdited")))
    return;
  const department =
    input.kind === "store"
      ? "المتجر"
  ...
  const path =
    input.kind === "kosha"
      ? `/admin/kosha-bookings?booking=${input.id}`
      : `/admin/orders?${input.kind === "store" ? "order" : "serviceOrder"}=${input.id}`;
  await sendTelegramMessage(
    messageBlock(
      "🛠️ <b>تم تعديل طلب</b>",
      [
...
export async function notifyTelegramLogin(user: {
  id: number;
  username: string;
...
export async function notifyTelegramPayment(
  payment: TelegramPaymentInput,
): Promise<void> {
  if (!(await eventEnabled(payment.event))) return;
  const title =
    payment.event === "managerApproval"
      ? "✅ <b>وافق المدير على استلام مبلغ</b>"
      : "💵 <b>تم استلام دفعة جديدة</b>";
  await sendTelegramMessage(
    messageBlock(
      title,
      [
      ...
      payment.entityPath
        ? adminLink(payment.entityPath)
        : adminLink("/admin/finance/master-cash"),
    ),
  );
}

export async function notifyTelegramDailyReport(
  report: TelegramDailyReportInput,
): Promise<void> {
  if (!(await eventEnabled(report.event))) return;
  const title =
    report.event === "dailyCashClosed"
      ? "🔒 <b>تم إغلاق الصندوق اليومي</b>"
      : "📊 <b>تم إنشاء تقرير مالي يومي</b>";
  await sendTelegramMessage(
    messageBlock(
      title,
      [
  ...
  try {
    ...
    await sendTelegramDocument(
      ...
      `التقرير المالي ${report.reportDate}`,
    );
  } catch (error) {
    console.warn("telegram daily report PDF generation failed", {
...
export async function sendTelegramTestPdf(): Promise<TelegramResult> {
  try {
    ...
    return sendTelegramDocument(
      pdf,
      "ajn-telegram-test.pdf",
      "اختبار ملف PDF من AJN",
    );
  } catch (error) {
