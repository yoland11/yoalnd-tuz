...
export function ensureRemotePrintingTables(): Promise<void> {
  if (!printTablesPromise) {
    printTablesPromise = (async () => {
      await db.execute(sql`create table if not exists print_agents (
      ...
      await db.execute(sql`create unique index if not exists print_agents_agent_id_unique_idx on print_agents(agent_id)`);
      await db.execute(sql`create unique index if not exists print_agents_token_hash_unique_idx on print_agents(token_hash) where token_hash is not null`);
      await db.execute(sql`create table if not exists printers (
      ...
      await db.execute(sql`create index if not exists print_jobs_queue_idx on print_jobs(computer_agent_id, status, next_attempt_at, requested_at)`);
      await db.execute(sql`create index if not exists print_jobs_invoice_idx on print_jobs(invoice_id, created_at)`);
    })().catch((error) => {
