...
export async function getEventBrainDashboard() {
  ...
  const [bookingsResult, overdueResult, tasksResult, stockResult, maintenanceResult, workloadResult, financeResult, payrollResult, attendanceResult, servicesResult, topEmployeeResult] = await Promise.all([
    ...
    db.execute(sql`SELECT (SELECT coalesce(current_balance::numeric,0)::float FROM master_cash_box WHERE code='MASTER' LIMIT 1) AS cashbox, coalesce(sum(amount::numeric) filter(where direction='revenue' and transaction_date=current_date AND coalesce(source_type,'') NOT IN ('company_loan','company_loan_repayment','employee_advance') AND transaction_type NOT IN ('company_loan_received','company_loan_repayment','employee_advance','employee_advance_repayment')),0)::float AS today_revenue, coalesce(sum(amount::numeric) filter(where direction='revenue' and transaction_date >= date_trunc('month', current_date) AND coalesce(source_type,'') NOT IN ('company_loan','company_loan_repayment','employee_advance') AND transaction_type NOT IN ('company_loan_received','company_loan_repayment','employee_advance','employee_advance_repayment')),0)::float AS month_revenue FROM financial_transactions WHERE approval_status='executed'`),
  ...
  overdue.forEach((row: any) => alerts.push({ id: `payment-${row.id}`, priority: row.event_date === new Date().toISOString().slice(0, 10) ? "critical" : "high", category: "payment_overdue", title: "دفعة عميل مستحقة", body: `${row.customer_name} عليه ${money(row.remaining).toLocaleString("ar-IQ-u-nu-latn")} د.ع للحجز ${row.booking_no}.`, href: `/admin/kosha-bookings?focus=${row.id}`, entityType: "kosha_booking", entityId: row.id }));
  ...
  if (n(payroll.pending) > 0) alerts.push({ id: "payroll-pending", priority: "medium", category: "payroll_pending", title: "رواتب بانتظار الإجراء", body: `يوجد ${money(payroll.pending).toLocaleString("ar-IQ-u-nu-latn")} د.ع من الرواتب غير المرحلة.`, href: "/admin/hr?tab=payroll" });
  if (n(finance.cashbox) < 0) alerts.push({ id: "cashbox-low", priority: "high", category: "cashbox_low", title: "رصيد الصندوق منخفض", body: `رصيد الصندوق الحالي ${money(finance.cashbox).toLocaleString("ar-IQ-u-nu-latn")} د.ع.`, href: "/admin/finance/master-cash" });
